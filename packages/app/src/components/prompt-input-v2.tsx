...
export function usePromptInputV2Controller(props: PromptInputV2ControllerProps): PromptInputV2ComposerController {
  ...
  const protocols = createMemo(() =>
    props.controls.model.selection.protocol.list().map((value) => ({
      id: value,
      label:
  ...
  const controller = createPromptInputV2Controller({
    ...
    view: {
      ...
      protocol: {
        title: () => language.t("provider.custom.protocol.label"),
        options: protocols,
        current: () => props.controls.model.selection.protocol.current() ?? "",
        onSelect: (value) =>
          props.controls.model.selection.protocol.set(
            value as "openai-responses" | "openai-compatible" | "anthropic-messages",
          ),
      },
      variant: {
