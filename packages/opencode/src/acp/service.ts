...
import { coalescePromptParts, promptContentToParts, type PromptPart } from "./content"
...
const textualMime = (mime: string) =>
  mime.startsWith("text/") ||
  mime === "application/json" ||
  mime === "application/xml" ||
  mime === "application/javascript"

const materializeLocalTextResources = Effect.fn("ACP.materializeLocalTextResources")(function* (
  parts: readonly PromptPart[],
) {
  const materialized = yield* Effect.forEach(
    parts,
    (part) => {
      if (part.type !== "file" || !part.url.startsWith("file:") || !textualMime(part.mime)) {
      ...
      return Effect.tryPromise({
        try: () => readFile(filepath, "utf8"),
          ...
          new ACPError.ServiceFailureError({
            safeMessage: "Failed to read local text resource",
            service: "filesystem",
  ...
  return coalescePromptParts(materialized)
})
...
export function make(input: {
  ...
  const directoryService = input.directory ?? makeDirectoryService(input.client)
  const registeredMcp = new Map<string, Set<string>>()
  const sessionSnapshots = new Map<string, Directory.Snapshot>()
  const events = input.connection
    ? ACPEvent.start({ client: input.client, connection: input.connection, session })
    : undefined
  ...
  const newSession = Effect.fn("ACP.newSession")(function* (params: NewSessionRequest) {
    ...
    yield* registerMcpServers(input.client, registeredMcp, params.cwd, state.id, params.mcpServers)
  ...
  const loadSession = Effect.fn("ACP.loadSession")(function* (params: LoadSessionRequest) {
    const snapshot = yield* directorySnapshot(params.cwd)
    yield* request(() => input.client.session.get({ sessionID: params.sessionId }), "session")
    const messages = yield* request(() => input.client.session.messages({ sessionID: params.sessionId }), "session")
  ...
  const listSessions = Effect.fn("ACP.listSessions")(function* (params: ListSessionsRequest) {
    ...
    const sessions = yield* request(
      () => input.client.session.list(params.cwd ? { directory: params.cwd } : {}),
      "session",
    )
    const serverEntries = sessions
  ...
  const resumeSession = Effect.fn("ACP.resumeSession")(function* (params: ResumeSessionRequest) {
    ...
    const messages = yield* request(
      () => input.client.session.messages({ sessionID: params.sessionId, limit: 20 }),
      "session",
    )
    const restored = restoreFromMessages(messages.map((item) => item.info))
    const model = restored.model ?? selectDefaultModel(snapshot)
  ...
  const abortBackingSession = Effect.fn("ACP.abortBackingSession")(function* (current: ACPSession.Info) {
    yield* request(() => input.client.session.interrupt({ sessionID: current.id }), "session").pipe(
      Effect.catch((error) =>
  ...
  const forkSession = Effect.fn("ACP.forkSession")(function* (params: ForkSessionRequest) {
    const snapshot = yield* directorySnapshot(params.cwd)
    const forked = yield* request(() => input.client.session.fork({ sessionID: params.sessionId }), "session")
    const messages = yield* request(() => input.client.session.messages({ sessionID: forked.id, limit: 20 }), "session")
    const restored = restoreFromMessages(messages.map((item) => item.info))
    const model = restored.model ?? selectDefaultModel(snapshot)
    const state = yield* session.load({
      id: forked.id,
  ...
  return {
    ...
    prompt: Effect.fn("ACP.prompt")(function* (params: PromptRequest) {
      ...
      const parts = yield* materializeLocalTextResources(promptContentToParts(params.prompt))
      ...
      if (!command) {
        ...
        yield* sendUsageUpdate(input.usage, input.client, input.connection, current.id, current.cwd)
        if (response === ACPClient.CompletionCancelled) return cancelledPromptResponse(params.messageId)
      ...
      if (command.name === "compact") {
        yield* request(() => input.client.session.compact({ sessionID: current.id }), "session")
      }
...
function makeDirectoryService(client: ACPClient.Interface) {
        ...
        Layer.succeed(
          ...
          Directory.Loader.of({
            load: (directory) =>
              request(
                () =>
                  ACPProfile.measure("acp.directory.load", async () => {
                    const catalog = await client.catalog.load(directory)
                    const defaultModelStarted = performance.now()
                    const defaultModel = defaultModelFromConfig(catalog.configuredModel, catalog.providers)
                    ACPProfile.duration("acp.directory.defaultModel.resolve", defaultModelStarted, {
                      configured: !!defaultModel,
                    })
                    const modes = catalog.agents
                    ...
                    return Directory.build({
                      ...
                      modes,
                      defaultModeID:
                        catalog.agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ??
                        "build",
...
function makeUsageService(client: ACPClient.Interface) {
  ...
  const contextLimit: UsageService.Interface["contextLimit"] = Effect.fn("ACP.promptUsage.contextLimit")(
    function* (params) {
      ...
      const next = client.catalog
        .load(params.directory)
        .then((catalog) => UsageService.findContextLimit(catalog.providers, params.providerID, params.modelID))
        .catch(() => undefined)
      limits.set(key, next)
  ...
  const sendUpdate: UsageService.Interface["sendUpdate"] = Effect.fn("ACP.promptUsage.sendUpdate")(function* (params) {
    const messages = yield* request(() => client.session.messages({ sessionID: params.sessionID }), "session").pipe(
...
function replayMessages(
  ...
  messages: readonly ACPClient.LegacySessionMessage[],
) {
...
type MessageInfo = {
  readonly role?: ACPClient.LegacyMessage["role"]
  readonly model?: Extract<ACPClient.LegacyMessage, { role: "user" }>["model"]
  readonly providerID?: Extract<ACPClient.LegacyMessage, { role: "assistant" }>["providerID"]
  readonly modelID?: Extract<ACPClient.LegacyMessage, { role: "assistant" }>["modelID"]
  readonly variant?: Extract<ACPClient.LegacyMessage, { role: "assistant" }>["variant"]
  readonly mode?: Extract<ACPClient.LegacyMessage, { role: "assistant" }>["mode"]
  readonly agent?: ACPClient.LegacyMessage["agent"]
}

type AssistantError = NonNullable<ACPClient.LegacyAssistantMessage["error"]>
type AssistantInfo = (UsageService.AssistantTokenCost & Pick<ACPClient.LegacyAssistantMessage, "error">) | undefined
...
function cancelledPromptResponse(messageId: string | null | undefined) {
  return {
    stopReason: "cancelled" as const,
    ...(messageId ? { userMessageId: messageId } : {}),
    _meta: {},
...
function sendUsageUpdate(
  ...
  return (usage ?? makeUsageService(client)).sendUpdate({
    connection,
    sessionID,
    directory,
  })
