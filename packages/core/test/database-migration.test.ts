...
describe("DatabaseMigration", () => {
  ...
  test("applies tracked migrations to an empty database", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(
          yield* db.get(sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'task_submission'`),
        ).toEqual({ name: "task_submission" })
        expect(
          yield* db.get(
            sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'task_notification_outbox'`,
          ),
        ).toEqual({ name: "task_notification_outbox" })
        expect(
          yield* db.get(
            sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'session_cancellation'`,
          ),
  ...
  test("defaults existing task submissions to parent completion delivery", async () => {
    await run(
      Effect.gen(function* () {
        const db = yield* makeDb
        yield* db.run(sql`
        ...
        yield* db.run(sql`
          INSERT INTO task_submission (
            ...
            description, prompt, agent, status, time_created
          ) VALUES ('sub_existing', 'ses_parent', 'msg_parent', 'call_existing', 'ses_child', 'msg_child', 'existing task', '{}', 'general', 'accepted', 1)
        `)

        yield* DatabaseMigration.applyOnly(db, [taskCompletionDeliveryMigration])
