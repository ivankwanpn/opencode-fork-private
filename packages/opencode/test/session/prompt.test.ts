...
import { SessionMessageTable, TaskSubmissionTable } from "@opencode-ai/core/session/sql"
...
it.instance("subtask child inherits parent session external_directory allow", () =>
  Effect.gen(function* () {
    ...
    const child = kids[0]!
    const { db } = yield* Database.Service
    const submission = (yield* db.select().from(TaskSubmissionTable).all())[0]
    expect(submission?.status).toBe("completed")
    const childInputID = submission?.child_input_id
    expect(
      (yield* MessageV2.filterCompactedEffect(child.id)).some(
        (message) => childInputID !== undefined && String(message.info.id) === childInputID,
      ),
    ).toBe(true)
