...
import { type CompatibleApi, type CompatibleImplementation } from "@/utils/server-compat"
...
function mergeInOrder<T extends { id: string }>(a: readonly T[], b: readonly T[]) {
  const items = new Map(a.map((item) => [item.id, item] as const))
  for (const item of b) items.set(item.id, item)
  return [...items.values()]
}
...
export function createServerSession(
  ...
  const projection = projectedClient as ProjectedSessionClient | undefined
  ...
  const resolveCurrentApi = () => {
    if (options?.apiForGeneration) return options.apiForGeneration()
    if (options?.api) return Promise.resolve(options.api)
  }
  const resolveCurrentSessionApi = () => {
    const current = resolveCurrentApi()
    if (current) return current.then((api) => api.session)
    if (sessionApi) return Promise.resolve(sessionApi)
  }
  ...
  const resolve = (sessionID: string, requestOptions?: { force?: boolean }) => {
    const cached = data.info[sessionID]
    if (cached && !requestOptions?.force) return Promise.resolve(cached)
    ...
    const request = options?.apiForGeneration
      ...
      : options?.api
        ? options.api.session.get({ sessionID }).then(normalizeSessionInfo)
        : sessionApi
          ? sessionApi.get({ sessionID }).then(normalizeSessionInfo)
          : projection!.session.get({ sessionID }).then((result) => {
  ...
  const confirmOptimisticPart = (sessionID: string, messageID: string, part: Part) => {
    ...
    items.set(messageID, {
      ...item,
      parts,
      confirmedParts: mergeInOrder(item.confirmedParts ?? [], [part]),
  ...
  const confirmOptimistic = (sessionID: string, messageID: string, confirmedParts: Part[]) => {
    ...
    items.set(messageID, {
      ...item,
      parts,
      confirmedParts: mergeInOrder(item.confirmedParts ?? [], confirmedParts),
      confirmedMessage: true,
    })
  }
  ...
  const fetchMessages = async (sessionID: string, limit: number, before?: string, onAttempt?: () => void) => {
    const currentMessageApi = options?.apiForGeneration
      ...
      : (options?.api?.message ?? messageApi)
    if (currentMessageApi) {
      const pageLimit = Math.min(limit, messageApiPageSize)
      const request = (cursor?: string) =>
        (options?.retry ?? retry)(() => {
          onAttempt?.()
          return currentMessageApi.list(
            cursor ? { sessionID, limit: pageLimit, cursor } : { sessionID, limit: pageLimit, order: "desc" },
          )
        })
      ...
      return {
        ...
        watermark: "watermark" in first && typeof first.watermark === "number" ? first.watermark : undefined,
      }
    }
    const response = await (options?.retry ?? retry)(() => {
      onAttempt?.()
      return projection!.session.messages({ sessionID, limit, before })
    })
  ...
  const fetchMessage = async (sessionID: string, messageID: string, onAttempt?: () => void) => {
    const currentSessionApi = options?.apiForGeneration
      ...
      : (options?.api?.session ?? sessionApi)
  ...
  const replaceParts = (
    ...
    for (const item of items) {
      ...
      const parts = reconcileFetched(fetched, data.part[item.id] ?? [], { touched, preserveOrder: true })
      if (!parts.length) {
  ...
  const refreshContext = (sessionID: string) => {
    const currentSessionApi = resolveCurrentSessionApi()
    if (!currentSessionApi) return Promise.resolve()
    return runInflight(contextLoads, sessionID, async () => {
      const active = generation(sessionID)
      const result = await (options?.retry ?? retry)(() => currentSessionApi.then((api) => api.context({ sessionID })))
      if (generations.get(sessionID) !== active) return
  ...
  const projectV2 = (reduction: V2SessionReduction) => {
    ...
    batch(() => {
      ...
      for (const messageID of touched) {
        ...
        const current = new Map((data.part[messageID] ?? []).map((part) => [part.id, part]))
        const ordered = next.flatMap((part) => {
          const value = current.get(part.id)
          return value ? [value] : []
        })
        if (ordered.length) setData("part", messageID, reconcile(ordered, { key: "id" }))
      }
    })
  }
  ...
  const applyV2 = (event: V2InputEvent, recovering = false) => {
        ...
        void sync(sessionID, { force: true })
          .then(async () => {
            ...
            const events = recovery.events
              ...
              .sort(
                (a, b) =>
                  ("durable" in a && a.durable ? a.durable.seq : -1) -
                  ("durable" in b && b.durable ? b.durable.seq : -1),
              )
            for (const pending of events) {
              if ("durable" in pending && pending.durable && pending.durable.seq <= (watermark ?? baseline)) continue
    ...
    if (event.type === "session.next.turn.started") turns.set(sessionID, { turnID: event.data.turnID, phase: "active" })
    ...
    if (event.type === "session.next.revert.staged" && info)
      remember({
        ...
        revert: {
          ...
          snapshot: event.data.revert.snapshot,
          diff: event.data.revert.diff,
        },
        time: { ...info.time, updated: event.data.timestamp },
      })
    if ((event.type === "session.next.revert.cleared" || event.type === "session.next.revert.committed") && info)
      remember({
        ...info,
        revert: undefined,
        time: { ...info.time, updated: event.data.timestamp },
    ...
    if (event.type === "session.next.revert.committed") void sync(sessionID, { force: true }).catch(() => {})
    if (
  ...
  const apply = (event: { type: string; properties?: unknown }) => {
    ...
    switch (event.type) {
      ...
      case "message.part.updated": {
        ...
        if (index !== -1) setData("part", part.messageID, index, reconcile(part))
        if (index === -1)
  ...
  return {
    ...
    optimistic: {
      add(input: { sessionID: string; message: Message; parts: Part[] }) {
        const parts = input.parts.filter((part) => !!part?.id && !SKIP_PARTS.has(part.type))
    ...
    async todo(sessionID: string, request?: { force?: boolean }) {
      ...
      const currentSession = options?.apiForGeneration
        ? options.apiForGeneration().then((api) => api.session)
        : options?.api
          ? Promise.resolve(options.api.session)
          : options?.currentSession
            ? Promise.resolve(options.currentSession)
            : undefined
      if (currentSession) {
        return runInflight(inflightTodo, sessionID, async () => {
          const active = generation(sessionID)
          const result = await (options?.retry ?? retry)(() => currentSession.then((api) => api.todo({ sessionID })))
          if (generations.get(sessionID) !== active) return
      ...
      return runInflight(inflightTodo, sessionID, () => {
        const active = generation(sessionID)
        return (options?.retry ?? retry)(() => projection.session.todo({ sessionID })).then((result) => {
          if (generations.get(sessionID) !== active) return
