...
import { Archive, Filesystem, Process } from "./runtime"
...
type SpawnOptions = NonNullable<Parameters<typeof LSPProcess.spawn>[1]>
function spawn(command: string, args: string[], options?: SpawnOptions): LSPProcess.Child
function spawn(command: string, options?: SpawnOptions): LSPProcess.Child
function spawn(command: string, argsOrOptions?: string[] | SpawnOptions, options?: SpawnOptions) {
  return Array.isArray(argsOrOptions)
    ? LSPProcess.spawn(command, argsOrOptions, options)
    : LSPProcess.spawn(command, [], argsOrOptions)
}

export interface Handle {
  readonly process: LSPProcess.Child
...
export type Root = (file: string, context: Context) => Promise<string | undefined>

const NearestRoot = (includePatterns: string[], excludePatterns?: string[]): Root => {
...
const StrictNearestRoot = (includePatterns: string[], excludePatterns?: string[]): Root => {
  return async (file, ctx) => {
    if (excludePatterns) {
      const excludedFiles = Filesystem.up({
        targets: excludePatterns,
...
export interface Info {
  ...
  readonly root: Root
  readonly spawn: (root: string, context: Context) => Promise<Handle | undefined>
}
...
export const CSharp: Info = {
  ...
  extensions: [".cs", ".csx"],
  async spawn(root, ctx) {
    const bin = await getRoslynLanguageServer(ctx.disableDownload)
    if (!bin) return
...
export const RustAnalyzer: Info = {
  ...
  root: async (file, ctx) => {
    ...
    while (currentDir !== path.dirname(currentDir)) {
      ...
      if (!currentDir.startsWith(ctx.projectDirectory)) break
    }
...
export const Nixd: Info = {
  ...
  root: async (file, ctx) => {
    ...
    if (ctx.projectDirectory && ctx.projectDirectory !== ctx.directory) return ctx.projectDirectory
...
export const builtins = [
  ...
  HLS,
  JuliaLS,
] as const satisfies ReadonlyArray<Info>
