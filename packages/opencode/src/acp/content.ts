...
export function coalescePromptParts(parts: readonly PromptPart[]): PromptPart[] {
  ...
  for (const part of parts) {
    ...
    if (
      previous?.type === "text" &&
      part.type === "text" &&
      previous.synthetic === part.synthetic &&
      previous.ignored === part.ignored
    ) {
      previous.text += part.text
      continue
    }
    result.push(part.type === "text" ? { ...part } : part)
  }
  return result
}

export function promptContentToParts(content: readonly ContentBlock[]): PromptPart[] {
  return coalescePromptParts(content.flatMap(contentBlockToParts))
}
