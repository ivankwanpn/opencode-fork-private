...
const TOOL_FILE_MIMES = new Set<string>(ProviderShared.IMAGE_MIMES)
const USER_FILE_MIMES = new Set<string>(["application/pdf"])
...
const lowerToolResultContentItem = Effect.fn("OpenAIResponses.lowerToolResultContentItem")(function* (
  ...
  const media = yield* ProviderShared.validateToolFile("OpenAI Responses", item, TOOL_FILE_MIMES)
  return { type: "input_image" as const, image_url: media.dataUrl }
})

const lowerToolResultUserFile = Effect.fn("OpenAIResponses.lowerToolResultUserFile")(function* (item: ToolFileContent) {
  const media = yield* ProviderShared.validateToolFile("OpenAI Responses", item, USER_FILE_MIMES)
  return { type: "input_file" as const, filename: item.name ?? "file.pdf", file_data: media.dataUrl }
})

const lowerToolResult = Effect.fn("OpenAIResponses.lowerToolResult")(function* (part: ToolResultPart) {
  // Text/json/error results are encoded as a plain string for backward
  // compatibility with existing cassettes and provider expectations.
  if (part.result.type !== "content") return { output: ProviderShared.toolResultText(part), user: [] }
  // Preserve the narrowed array element type when compiled through a consumer package.
  const content: ReadonlyArray<ToolContent> = part.result.value
  const partition = ProviderShared.partitionToolContent(
    content,
    (item) => TOOL_FILE_MIMES.has(item.mime.toLowerCase()),
    (item) => USER_FILE_MIMES.has(item.mime.toLowerCase()),
  )
  if (partition.unsupported.length > 0)
    return yield* invalid(`OpenAI Responses does not support media type ${partition.unsupported[0]!.mime}`)
  return {
    output: yield* Effect.forEach(partition.tool, lowerToolResultContentItem),
    user: yield* Effect.forEach(partition.user, lowerToolResultUserFile),
  }
})
...
const lowerMessages = Effect.fn("OpenAIResponses.lowerMessages")(function* (
  ...
  const flushUserFiles = () => {
    if (pendingUserFiles.length === 0) return
    input.push({ role: "user", content: pendingUserFiles.splice(0) })
    return true
  }

  for (const message of request.messages) {
    const userFilesFlushed = message.role === "tool" ? false : flushUserFiles()
    ...
    for (const part of message.content) {
      ...
      pendingUserFiles.push(...lowered.user)
    }
  }
...
const providerErrorMessage = (event: OpenAIResponsesEvent, fallback: string): string => {
  ...
  const base = message && code ? `${code}: ${message}` : message || code || fallback
  const status = event.response?.status
  const details = [
    typeof status === "string" || typeof status === "number" ? `status=${status}` : undefined,
    event.response?.id ? `response_id=${event.response.id}` : undefined,
    event.param || nested?.param ? `param=${event.param || nested?.param}` : undefined,
