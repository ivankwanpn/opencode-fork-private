  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachHeaders = args.attach
        ...
        : undefined
      const attachClients = (dir?: string) =>
        createAttachClients({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(
        sdk: OpencodeClient,
        native: ReturnType<typeof OpenCode.make>,
      ): Promise<SessionInfo | undefined> {
        if (args.session) {
          const current = await native.sessions.get({ sessionID: args.session }).catch(() => undefined)
          ...
          if (args.fork) {
            const forked = await native.sessions.fork({ sessionID: args.session })

            return {
              id: forked.id,
              title: forked.title ?? current.title,
              directory: forked.location.directory ?? current.location.directory,
            }
          }
        ...
        if (base && args.fork) {
          const forked = await native.sessions.fork({ sessionID: base.id })

          return {
            id: forked.id,
            title: forked.title ?? base.title,
            directory: forked.location.directory ?? base.directory,
          }
        }
      ...
      async function share(sdk: OpencodeClient, native: ReturnType<typeof OpenCode.make>, sessionID: string) {
        const cfg = await sdk.config.get()
        if (!cfg.data) return
        if (cfg.data.share !== "auto" && !flags.autoShare && !args.share) return
        const res = await native.sessions.share({ sessionID }).catch((error) => {
        ...
        if (!("error" in res)) {
          UI.println(UI.Style.TEXT_INFO_BOLD + "~  " + res.url)
        }
      }
      ...
      async function createFreshSession(
        ...
        void share(sdk, native, id).catch(() => {})
        return {
          id,
      ...
      async function execute(clients: ReturnType<typeof createAttachClients>) {
        const sdk = clients.sdk
        const sess = await session(sdk, clients.native)
        if (!sess?.id) {
          UI.error("Session not found")
          process.exit(1)
        }
        ...
        async function loop(client: OpencodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
          ...
          for await (const event of events.stream) {
            ...
            if (event.type === "session.next.error") {
              ...
              if (props.error.data && typeof props.error.data === "object" && "message" in props.error.data) {
        ...
        const active = args.attach
          ? rebindAttachClients(clients, {
              baseUrl: args.attach,
              directory: cwd,
              headers: attachHeaders,
            })
          : clients
        const client = active.sdk

        // Validate agent if specified
        const agent = await pickAgent(client)

        await share(client, active.native, sessionID)
        ...
        if (!interactive) {
          ...
          if (args.command) {
            ...
            try {
              await active.native.sessions.command({
                sessionID,
                agent,
                model: model
                  ? {
              ...
              await active.native.sessions.wait({ sessionID })
            } catch (error) {
      ...
      if (interactive && !args.attach && !args.session && !args.continue) {
        ...
        const fetchFn = (async (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          return Server.Native().app.fetch(new Request(request, { headers }))
      ...
      if (args.attach) {
        return await execute(attachClients(directory))
      }
