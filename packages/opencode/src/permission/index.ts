...
export function replyCompatible(service: Interface, input: PermissionV1.ReplyInput) {
  return service.reply(input).pipe(
    Effect.catchTag("Permission.NotFoundError", (legacyError) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          const canonical = yield* PermissionV2.Service
          yield* canonical.reply({
            requestID: PermissionV2.ID.create(input.requestID),
            reply: input.reply,
          ...
          Effect.catchTag("PermissionV2.NotFoundError", () => Effect.fail(legacyError)),
        )
      }),
    ),
  )
