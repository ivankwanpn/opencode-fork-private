...
test("provider errors identify the selected custom protocol and model", async () => {
  const { published, publisher } = capture("openai-responses")
  await Effect.runPromise(
    publisher.publish(LLMEvent.providerError({ message: "upstream_error: Upstream request failed" })),
  )

  expect(published.find((event) => event.type === "session.next.step.failed.2")?.data).toMatchObject({
    error: {
      message:
