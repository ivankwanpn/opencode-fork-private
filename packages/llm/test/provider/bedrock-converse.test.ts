...
describe("Bedrock Converse route", () => {
  ...
  it.effect("keeps separate consecutive tool results contiguous before ordered documents", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<BedrockConverse.BedrockConverseBody>(
        LLM.request({
          model,
          messages: [
            Message.assistant([
              ToolCallPart.make({ id: "tool_1", name: "read", input: {} }),
              ToolCallPart.make({ id: "tool_2", name: "fetch", input: {} }),
            ]),
            Message.tool({
              id: "tool_1",
              name: "read",
            ...
            Message.tool({
              ...
              resultType: "content",
              result: [
                { type: "text", text: "Table fetched." },
                { type: "file", uri: "data:text/csv;base64,Q1NWREFUQQ==", mime: "text/csv", name: "table.csv" },
              ],
            }),
          ],
          cache: "none",
      ...
      expect(prepared.body.messages.slice(-3)).toEqual([
        {
          role: "user",
          content: [
            {
        ...
        {
          role: "user",
          content: [{ toolResult: { toolUseId: "tool_2", content: [{ text: "Table fetched." }], status: "success" } }],
        },
        {
          role: "user",
          content: [
            { document: { format: "pdf", name: "report.pdf", source: { bytes: "UERGREFUQQ==" } } },
            { document: { format: "csv", name: "table.csv", source: { bytes: "Q1NWREFUQQ==" } } },
          ],
        },
      ])
  ...
  it.effect("propagates BedrockMedia invalid tool-result MIME errors", () =>
    Effect.gen(function* () {
        ...
        LLM.request({
          ...
          messages: [
            Message.tool({
              ...
              name: "read",
              resultType: "content",
              result: [{ type: "file", uri: "data:application/x-tar;base64,QUJDRA==", mime: "application/x-tar" }],
            }),
          ],
        }),
      ).pipe(Effect.flip)
