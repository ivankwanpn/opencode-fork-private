...
test("selects a dynamically discovered utility model for small-model hooks", async () => {
  globalThis.fetch = mock(() =>
    Promise.resolve(
      new Response(
        JSON.stringify({
  ...
  await hooks.provider!.models!(
    { models: {}, options: {} } as never,
    { auth: { type: "oauth", refresh: "token" } } as never,
  )
  const output: { model?: { id: string } } = {}
  await hooks["experimental.provider.small_model"]!(
    { provider: { id: "github-copilot" } } as never,
    output as never,
  )
