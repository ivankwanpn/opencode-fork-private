...
async function withTrackedInvalidations(
  run: (
    invalidations: () => number,
  ) => Effect.Effect<void, unknown, Config.Service | FSUtil.Service | ChildProcessSpawner | Scope.Scope>,
) {
  let invalidations = 0
  const cachedInvalidateWithTTL = Effect.cachedInvalidateWithTTL
  const cached = spyOn(Effect, "cachedInvalidateWithTTL")
  const tracked = <A, E, R>(effect: Effect.Effect<A, E, R>, ttl: Duration.Input) =>
    cachedInvalidateWithTTL(effect, ttl).pipe(
      Effect.map(
        ([get, invalidate]) =>
          [
            get,
          ...
          ] as [typeof get, Effect.Effect<void>],
      ),
    )
  function trackedCachedInvalidateWithTTL(
    ttl: Duration.Input,
  ): <A, E, R>(effect: Effect.Effect<A, E, R>) => Effect.Effect<[Effect.Effect<A, E, R>, Effect.Effect<void>]>
  function trackedCachedInvalidateWithTTL<A, E, R>(
    effect: Effect.Effect<A, E, R>,
    ttl: Duration.Input,
  ): Effect.Effect<[Effect.Effect<A, E, R>, Effect.Effect<void>]>
  function trackedCachedInvalidateWithTTL<A, E, R>(
    effectOrTTL: Effect.Effect<A, E, R> | Duration.Input,
    ttl?: Duration.Input,
  ) {
    if (ttl === undefined) {
      return <A2, E2, R2>(effect: Effect.Effect<A2, E2, R2>) => tracked(effect, effectOrTTL as Duration.Input)
    }
    return tracked(effectOrTTL as Effect.Effect<A, E, R>, ttl)
  }
  cached.mockImplementation(trackedCachedInvalidateWithTTL)
  try {
    await Effect.runPromise(run(() => invalidations).pipe(Effect.scoped, Effect.provide(configLayer())))
  } finally {
    cached.mockRestore()
  }
}
...
describe("updateGlobalProvider", () => {
  const replacement = ConfigProviderV1.Info.make({
    npm: "@ai-sdk/openai-compatible",
    name: "Replacement",
    options: { baseURL: "https://replacement.example/v1" },
    models: { fresh: { name: "Fresh" } },
  })

  it.effect("replaces exact JSON fields without retaining stale provider data", () =>
    withGlobalConfig(
      {
        config: {
          username: "unrelated",
          disabled_providers: ["custom", "other"],
          provider: {
            custom: {
              npm: "@ai-sdk/openai-compatible",
              name: "Stale",
              env: ["STALE_KEY"],
              options: { baseURL: "https://stale.example", headers: { "x-stale": "yes" } },
              models: { stale: { name: "Stale" } },
            },
            other: { npm: "@ai-sdk/openai", name: "Other", models: { kept: { name: "Kept" } } },
          },
        },
      },
      ({ dir }) =>
        Effect.gen(function* () {
          ...
          const writes = spyOn(fsSvc, "writeFileString")
          const result = yield* Config.use.updateGlobalProvider({
            providerID: "custom",
            provider: replacement,
          })

          const written = (yield* FSUtil.use.readJson(path.join(dir, "opencode.json"))) as ConfigV1.Info
          expect(result.changed).toBe(true)
          expect(written.username).toBe("unrelated")
          expect(written.provider?.other?.models?.kept?.name).toBe("Kept")
          expect(written.provider?.custom).toEqual(replacement)
          expect(written.provider?.custom?.env).toBeUndefined()
  ...
  it.effect("preserves JSONC comments and unrelated text while replacing and deleting exact fields", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped()
      const file = path.join(dir, "opencode.jsonc")
  ...
  "username": "unrelated",
  "provider": {
    // preserve the unrelated provider
    "other": { "npm": "@ai-sdk/openai", "name": "Other", "models": { "kept": { "name": "Kept" } } },
    "custom": { "npm": "@ai-sdk/openai-compatible", "name": "Stale", "env": ["STALE_KEY"], "models": { "stale": { "name": "Stale" } } }
  },
  "disabled_providers": ["custom", "other"]
}`,
      )
      ...
      yield* withGlobalConfigDir(
        dir,
        Effect.gen(function* () {
          const first = yield* Config.use.updateGlobalProvider({
            providerID: "custom",
            provider: replacement,
          ...
          const afterReplace = yield* FSUtil.use.readFileString(file)
          const parsed = ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(afterReplace, file), file)
          expect(afterReplace).toContain("// keep this account note")
          expect(afterReplace).toContain("// preserve the unrelated provider")
          expect(parsed.username).toBe("unrelated")
          expect(parsed.provider?.other?.models?.kept?.name).toBe("Kept")
          expect(parsed.provider?.custom).toEqual(replacement)
          expect(parsed.disabled_providers).toEqual(["other"])

          const second = yield* Config.use.updateGlobalProvider({
            providerID: "custom",
          })
          const afterDelete = yield* FSUtil.use.readFileString(file)
          const deleted = ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(afterDelete, file), file)
          expect(second.changed).toBe(true)
          expect(afterDelete).toContain("// keep this account note")
          expect(deleted.provider?.custom).toBeUndefined()
          expect(deleted.provider?.other).toBeDefined()
          expect(deleted.disabled_providers).toBeUndefined()
        }),
      )
    }),
  )

  test("deleting absent JSON fields is byte-for-byte unchanged with no write or invalidation", async () =>
    withTrackedInvalidations((invalidations) =>
      withGlobalConfig({ config: { username: "kept" } }, ({ dir }) =>
        Effect.gen(function* () {
          ...
          const result = yield* Config.use.updateGlobalProvider({ providerID: "absent" })
          const after = yield* FSUtil.use.readFileString(file)

          expect(result.changed).toBe(false)
          expect(result.info).toEqual(ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(before, file), file))
          expect(after).toBe(before)
          expect(JSON.parse(after)).toEqual(JSON.parse(before))
          expect(JSON.parse(after)).not.toHaveProperty("provider")
          expect(JSON.parse(after)).not.toHaveProperty("disabled_providers")
          expect(writes).toHaveBeenCalledTimes(0)
          expect(invalidations() - beforeInvalidations).toBe(0)
          writes.mockRestore()
        }),
      ),
    ))

  test("JSON exact deletes retain real parents and disabled absent combinations count once", async () =>
    withTrackedInvalidations((invalidations) =>
      withGlobalConfig(
        {
          config: {
            provider: { custom: replacement },
          },
        },
        ({ dir }) =>
          Effect.gen(function* () {
            ...
            const deleted = yield* Config.use.updateGlobalProvider({ providerID: "custom" })
            expect(deleted.changed).toBe(true)
            expect(deleted.info.provider).toEqual({})
            expect(writes).toHaveBeenCalledTimes(1)
            expect(invalidations() - beforeInvalidations).toBe(1)

            const addedDisabled = yield* Config.use.updateGlobalProvider({
              providerID: "custom",
              disabledProviders: [],
            })
            expect(addedDisabled.changed).toBe(true)
            expect(writes).toHaveBeenCalledTimes(2)
            expect(invalidations() - beforeInvalidations).toBe(2)

            const unchangedDisabled = yield* Config.use.updateGlobalProvider({
              providerID: "custom",
              disabledProviders: [],
            })
            expect(unchangedDisabled.changed).toBe(false)
            expect(writes).toHaveBeenCalledTimes(2)
            expect(invalidations() - beforeInvalidations).toBe(2)

            const removedDisabled = yield* Config.use.updateGlobalProvider({ providerID: "custom" })
            expect(removedDisabled.changed).toBe(true)
            expect(removedDisabled.info.disabled_providers).toBeUndefined()
            expect(writes).toHaveBeenCalledTimes(3)
            expect(invalidations() - beforeInvalidations).toBe(3)
            expect(yield* FSUtil.use.readJson(path.join(dir, "opencode.json"))).toEqual({
              $schema: "https://opencode.ai/config.json",
              provider: {},
            })
            writes.mockRestore()
  ...
  test("JSONC changed and unchanged updates perform exact write and invalidation counts", async () =>
    withTrackedInvalidations((invalidations) =>
      Effect.gen(function* () {
        const dir = yield* tmpdirScoped()
        const file = path.join(dir, "opencode.jsonc")
  ...
  // preserved
  "provider": {
    "custom": { "npm": "@ai-sdk/openai-compatible", "name": "Stale", "models": {} }
  }
}`,
        )
        yield* withGlobalConfigDir(
          ...
          Effect.gen(function* () {
            ...
            const writes = spyOn(fsSvc, "writeFileString")
            const beforeInvalidations = invalidations()
            const input = { providerID: "custom", provider: replacement } as const

            const changed = yield* Config.use.updateGlobalProvider(input)
            expect(changed.changed).toBe(true)
            expect(writes).toHaveBeenCalledTimes(1)
            expect(invalidations() - beforeInvalidations).toBe(1)

            const unchanged = yield* Config.use.updateGlobalProvider(input)
            expect(unchanged.changed).toBe(false)
            expect(writes).toHaveBeenCalledTimes(1)
            expect(invalidations() - beforeInvalidations).toBe(1)
            expect(yield* FSUtil.use.readFileString(file)).toContain("// preserved")
            writes.mockRestore()
          }),
        )
      }),
  ...
  test("JSONC validation failure leaves bytes intact before any write or invalidation", async () =>
    withTrackedInvalidations((invalidations) =>
      Effect.gen(function* () {
        const dir = yield* tmpdirScoped()
        const file = path.join(dir, "opencode.jsonc")
  ...
  "username": "kept"
        ...
        yield* FSUtil.use.writeWithDirs(file, before)
        yield* withGlobalConfigDir(
          dir,
          Effect.gen(function* () {
            const fsSvc = yield* FSUtil.Service
            const writes = spyOn(fsSvc, "writeFileString")
            const beforeInvalidations = invalidations()
            const exit = yield* Config.use
              .updateGlobalProvider({
                providerID: "custom",
                provider: { npm: 42 } as unknown as ConfigProviderV1.Info,
              })
              .pipe(Effect.exit)
  ...
  test("JSONC write defect is surfaced without writing bytes or invalidating", async () =>
    withTrackedInvalidations((invalidations) =>
      Effect.gen(function* () {
        const dir = yield* tmpdirScoped()
        const file = path.join(dir, "opencode.jsonc")
  ...
  "username": "kept"
        ...
        yield* withGlobalConfigDir(
          ...
          Effect.gen(function* () {
            const fsSvc = yield* FSUtil.Service
            const writes = spyOn(fsSvc, "writeFileString").mockImplementation(() =>
              Effect.die(new Error("simulated write defect")),
            )
            const beforeInvalidations = invalidations()
            const exit = yield* Config.use
              .updateGlobalProvider({ providerID: "custom", provider: replacement })
              .pipe(Effect.exit)
