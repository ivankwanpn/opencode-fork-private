      ...
      Effect.gen(function* () {
        ...
        const runPromptTurn = Effect.fn("RunStreamTransport.runPromptTurn")(function* (next: SessionTurnInput) {
          ...
          const commandFiles = command
            ...
            : []
          const requiresLegacyCommand = commandFiles.some((file) => {
            const id = "id" in file ? file.id : undefined
            const source = "source" in file ? file.source : undefined
            return id !== undefined || (source !== undefined && source.type !== "resource")
          })
          const send =
            next.prompt.mode === "shell"
              ...
              : command
                ? Effect.sync(() => {
                    ...
                    Effect.andThen(
                      Effect.promise(async () => {
                        if (!native || requiresLegacyCommand) {
                          await input.sdk.session.command(
                            {
                              sessionID: input.sessionID,
                              messageID: next.prompt.messageID,
                              agent: next.agent,
                              ...
                              return {
                                ...
                                resource:
                                  source?.type === "resource"
                                    ? { clientName: source.clientName, uri: source.uri }
                                    : undefined,
                              }
                            }),
                          },
