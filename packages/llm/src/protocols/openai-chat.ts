...
const lowerToolMessages = Effect.fn("OpenAIChat.lowerToolMessages")(function* (message: OpenAIChatRequestMessage) {
  ...
  for (const part of message.content) {
    ...
    const partition = ProviderShared.partitionToolContent(
      content,
      (item) => IMAGE_MIMES.has(item.mime.toLowerCase()),
      (_item: ToolFileContent) => false,
    )
    if (partition.unsupported.length > 0)
      return yield* invalid(`OpenAI Chat does not support media type ${partition.unsupported[0]!.mime}`)
    const text = partition.tool.filter((item) => item.type === "text").map((item) => item.text)
    messages.push({ role: "tool", tool_call_id: part.id, content: text.join("\n") })
    const files = partition.tool.filter((item): item is ToolFileContent => item.type === "file")
    images.push(
...
const lowerMessages = Effect.fn("OpenAIChat.lowerMessages")(function* (request: LLMRequest) {
  ...
  for (const message of request.messages) {
    const imagesFlushed = message.role === "tool" ? false : flushImages()
...
const step = (state: ParserState, event: OpenAIChatEvent) =>
  Effect.gen(function* () {
    ...
    const outputStarted = state.lifecycle.text.size > 0 || Object.keys(state.tools).length > 0
    if (delta?.reasoning_content && !outputStarted)
