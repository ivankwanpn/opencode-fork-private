...
const Loaded = { loaded: Schema.Array(AbsolutePath) }
export const Output = Schema.Union([
  Schema.Struct({ ...FileSystem.Content.fields, ...Loaded }),
  Schema.Struct({ ...ReadToolFileSystem.TextPage.fields, ...Loaded }),
  Schema.Struct({ ...ReadToolFileSystem.ListPage.fields, ...Loaded }),
])
        ...
        [name]: Tool.make({
          ...
          execute: (input, context) => {
            return Effect.gen(function* () {
              ...
              if (type === "directory")
                return { ...(yield* reader.list(absolute, { offset: input.offset, limit: input.limit })), loaded: [] }
              ...
              if ("encoding" in content && content.encoding === "base64" && SUPPORTED_IMAGE_MIMES.has(content.mime)) {
                ...
                return { ...normalized, loaded: [absolute] }
              }
              if ("encoding" in content && content.encoding === "base64")
                return yield* Effect.fail(new ReadToolFileSystem.BinaryFileError({ resource }))
              return { ...content, loaded: [absolute] }
            }).pipe(
