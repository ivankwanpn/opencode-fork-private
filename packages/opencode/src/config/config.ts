...
export interface Interface {
  ...
  readonly updateGlobalProvider: (input: {
...
function replaceJsonc(input: string, value: unknown, path: string[]): string {
  if (value === undefined) {
    ...
    if (!root || !findNodeAtLocation(root, path)) return input
  }
  return applyEdits(
    input,
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const writeGlobalFile = Effect.fnUntraced(function* (input: {
      readonly json: (current: Info) => Info
      readonly jsonc: (current: string) => string
    }) {
      const file = globalConfigFile()
      ...
      const updated = file.endsWith(".jsonc")
        ? input.jsonc(before)
        : (() => {
            const current = ConfigParse.schema(ConfigV1.Info, ConfigParse.jsonc(before, file), file)
            const next = input.json(current)
            if (JSON.stringify(next) === JSON.stringify(current)) return before
    ...
    const updateGlobal = Effect.fn("Config.updateGlobal")(function* (config: Info) {
      ...
      return yield* writeGlobalFile({
        json: (existing) => mergeDeep(writable(existing), patch),
        jsonc: (existing) => patchJsonc(existing, patch),
      })
    })

    const updateGlobalProvider = Effect.fn("Config.updateGlobalProvider")(function* (input: {
      readonly providerID: string
      readonly provider?: ConfigProviderV1.Info
      readonly disabledProviders?: readonly string[]
    }) {
      return yield* writeGlobalFile({
        json: (existing) => {
          const next = writable(existing)
          if (input.provider === undefined && next.provider?.[input.providerID] !== undefined) {
            const providers = { ...next.provider }
            delete providers[input.providerID]
            next.provider = providers
          }
          if (input.provider !== undefined) {
            next.provider = { ...next.provider, [input.providerID]: input.provider }
          }
          if (input.disabledProviders === undefined) delete next.disabled_providers
          if (input.disabledProviders !== undefined) next.disabled_providers = [...input.disabledProviders]
          return next
        },
        jsonc: (existing) => {
          const provider = replaceJsonc(existing, input.provider, ["provider", input.providerID])
          return replaceJsonc(provider, input.disabledProviders, ["disabled_providers"])
        },
      })
    })
