...
type DeltaPartType = Extract<ACPClient.LegacyPart, { type: "text" | "reasoning" }>["type"]
...
function createEventStream() {
  const queue: ACPClient.EventEnvelope[] = []
  const waiters: Array<(value: ACPClient.EventEnvelope | undefined) => void> = []
  const state = { closed: false }

  const push = (event: ACPClient.EventEnvelope) => {
  ...
  const stream = async function* (signal?: AbortSignal) {
    while (true) {
      ...
      const value = await new Promise<ACPClient.EventEnvelope | undefined>((resolve) => {
...
function createHarness(messages: Record<string, ACPClient.LegacySessionMessage> = {}) {
  ...
  const client = makeClient({
    events: {
      subscribe: (options) => {
        ...
        return events.stream(options?.signal)
      },
    },
    session: {
      message: (input: { messageID: string }) => {
        calls.message++
        return Promise.resolve(messages[input.messageID])
      },
...
function assistantMessage(sessionID: string, messageID: string, partID: string, type: DeltaPartType) {
  ...
  } satisfies ACPClient.LegacySessionMessage
}

function assistantToolMessage(part: ACPClient.LegacyToolPart) {
  return {
    info: {
      id: part.messageID,
      sessionID: part.sessionID,
...
function completedTool(
  ...
  callID: string,
  output = "done",
  attachments: Extract<ACPClient.LegacyToolPart["state"], { status: "completed" }>["attachments"] = [],
  options: {
...
async function createKnownSession(
  ...
  part: {
    messageId: string
    partId: string
    partType: ACPClient.LegacyPart["type"]
    role?: ACPClient.LegacyMessage["role"]
  },
) {
...
describe("acp event routing", () => {
  ...
  it("does not call client.session.message repeatedly when metadata is known", async () => {
    const harness = createHarness()
    await createKnownSession(harness.session, "ses_a", { messageId: "msg_a", partId: "part_a", partType: "text" })
  ...
  it("replays loaded session messages sequentially and continues after update failures", async () => {
    ...
    const service = ACPService.make({
      client: makeClient({
        events: {
          subscribe: (options) => events.stream(options?.signal),
        },
        session: {
          get: () => Promise.resolve({ id: "ses_loaded" } as ACPClient.SessionInfo),
          messages: () =>
