...
export function isV1(input: unknown) {
  ...
  const mcp = (input as Record<string, unknown>).mcp
  if (typeof mcp !== "object" || mcp === null || Array.isArray(mcp)) return false
  return Object.values(mcp).some(
    (server) =>
      typeof server === "object" &&
      server !== null &&
      !Array.isArray(server) &&
      ("type" in server || "enabled" in server),
  )
}
  ...
  return {
    ...
    provider_filter:
      info.enabled_providers === undefined && info.disabled_providers === undefined
        ? undefined
        : {
            enabled: info.enabled_providers,
            disabled: info.disabled_providers,
          },
  ...
  const capabilities =
    ...
    info.interleaved !== undefined
      ? {
          ...
          ...(info.attachment === undefined ? {} : { attachment: info.attachment }),
