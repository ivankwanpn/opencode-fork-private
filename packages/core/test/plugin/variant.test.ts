...
describe("VariantPlugin", () => {
  ...
  test("uses models.dev effort options for OpenAI-compatible providers", () => {
    const model = ModelV2.Info.make({
      ...ModelV2.Info.empty(ProviderV2.ID.make("deepseek"), ModelV2.ID.make("deepseek-v4-pro")),
      api: {
  ...
  test("keeps an explicit empty models.dev option list empty", () => {
    const model = ModelV2.Info.make({
      ...ModelV2.Info.empty(ProviderV2.ID.opencode, ModelV2.ID.make("glm-5.2")),
      api: {
        id: ModelV2.ID.make("glm-5.2"),
        type: "aisdk",
        package: "@ai-sdk/openai-compatible",
