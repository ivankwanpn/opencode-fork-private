...
export function fromToolOverrides(
...
export interface Interface {
  readonly ask: (input: AssertInput) => EffectRuntime.Effect<AskResult, SessionCommand.NotFoundError>
  readonly assert: (input: AssertInput) => EffectRuntime.Effect<void, Error | SessionCommand.NotFoundError>
...
const layer = Layer.effect(
  ...
  EffectRuntime.gen(function* () {
    ...
    const evaluateRequest = EffectRuntime.fnUntraced(function* (input: AssertInput) {
      const result = yield* evaluateInput(input)
      const value = request(input)
      const status = PluginRuntime.mutable(result.effect)
      yield* plugins.run(PluginRuntime.HookName.permissionAsk, {
        request: value,
        status: status.value,
      })
      return { effect: status.get(), request: value, rules: result.rules }
    })
    ...
    const ask = EffectRuntime.fn("PermissionV2.ask")(function* (input: AssertInput) {
      const result = yield* evaluateRequest(input)
      if (result.effect === "ask") yield* create(result.request, input.agent)
      return { id: result.request.id, effect: result.effect }
    })
    ...
    const assert = EffectRuntime.fn("PermissionV2.assert")((input: AssertInput) =>
      EffectRuntime.uninterruptibleMask((restore) =>
        EffectRuntime.gen(function* () {
          ...
          const item = yield* create(result.request, input.agent)
...
export const node = makeLocationNode({
  service: Service,
  layer,
  deps: [EventV2.node, Location.node, AgentV2.node, PluginRuntime.node, SessionStore.node, PermissionSaved.node],
})
