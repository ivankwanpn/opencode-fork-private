...
import { OAUTH_DUMMY_KEY } from "../../compat/auth-wire"
import os from "os"
import { setTimeout as sleep } from "node:timers/promises"
import { createServer } from "http"
import { OpenAIWebSocketPool } from "./ws-pool"
import { OauthCallbackPage } from "@opencode-ai/core/oauth/page"
import { CODEX_MODELS_ENDPOINT, fetchCodexModels } from "@opencode-ai/core/plugin/provider/codex-models"
...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  const codexModelsEndpoint = options.codexModelsEndpoint ?? deriveModelsEndpoint(codexApiEndpoint)
  ...
  return {
    ...
    provider: {
      ...
      async models(provider, ctx) {
        ...
        const auth = ctx.auth as typeof ctx.auth & { accountId?: string }
        let access = auth.access
        let accountId = auth.accountId
        if (!access || (typeof auth.expires === "number" && auth.expires < Date.now())) {
          const refreshed = auth.refresh
            ? await refreshAccessToken(auth.refresh, issuer).catch(() => undefined)
            : undefined
          if (refreshed) {
            access = refreshed.access_token
            accountId = extractAccountId(refreshed) ?? accountId
        ...
        if (!access) return lastLiveModels ?? provider.models

        try {
          lastLiveModels = await fetchCodexModels({ access, accountId }, { endpoint: codexModelsEndpoint })
        } catch {
          if (!lastLiveModels) return provider.models
        }

        const live = new Map(lastLiveModels?.map((model) => [model.id, model]))
        const existing = new Set(Object.values(provider.models).map((model) => model.api.id))
        const template = Object.values(provider.models)[0]
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => live.has(model.api.id))
            .map(([modelID, model]) => {
              const liveModel = live.get(model.api.id)
              return [
                modelID,
                {
                  ...model,
                  ...
                  limit: liveModel?.context ? { ...model.limit, context: liveModel.context } : model.limit,
                },
              ]
            })
            .concat(
              template
                ? Array.from(live.values())
                    ...
                    .map((liveModel) => [
                      ...
                      {
                        ...template,
                        id: ModelV2.ID.make(liveModel.id),
                        name: liveModel.name ?? liveModel.id,
                        api: { ...template.api, id: liveModel.id },
                        cost: {
                          input: 0,
                          output: 0,
                          cache: { read: 0, write: 0 },
                        },
                        limit: liveModel.context ? { ...template.limit, context: liveModel.context } : template.limit,
                      },
                    ])
                : [],
            ),
    ...
    auth: {
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          baseURL: codexApiEndpoint.replace(/\/responses\/?$/, ""),
...
function deriveModelsEndpoint(endpoint: string) {
  if (endpoint.endsWith("/responses")) return endpoint.slice(0, -"/responses".length) + "/models"
  return CODEX_MODELS_ENDPOINT
}
