...
import {
  ...
  type ProviderModel,
} from "@opencode-ai/core/provider-models"
...
export const Model = Schema.Struct({
  ...
  protocols: optional(Schema.Array(ModelV2.Protocol)),
...
export const Info = Schema.Struct({
  ...
  auth: optional(Schema.Literals(["env", "key", "oauth"])),
...
async function discoverLiveModels(provider: Info) {
  const template = Object.values(provider.models)[0]
  const packageName = template?.api.npm
  const baseURL =
    typeof provider.options.baseURL === "string" && provider.options.baseURL.trim().length > 0
      ? provider.options.baseURL
      : template?.api.url
  if (!supportsProviderModelDiscovery({ providerID: provider.id, packageName, baseURL })) return

  const apiKey =
    provider.key ??
    (typeof provider.options.apiKey === "string" && !provider.options.apiKey.startsWith("{env:")
      ? provider.options.apiKey
      : undefined)
  const headers = isRecord(provider.options.headers)
    ? Object.fromEntries(
        ...
        yield* Effect.promise(async () => {
          const results = await Promise.all(
            Object.values(providers).map(async (provider) => {
              const models = await discoverLiveModels(provider).catch(() => undefined)
