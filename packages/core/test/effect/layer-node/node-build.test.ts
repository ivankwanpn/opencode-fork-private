...
describe("node build", () => {
  ...
  test("builds a location service map required by a replacement graph", async () => {
    const value = LayerNode.unbound(Value, Node.tags.values.global)
    const replacement = Node.makeGlobalNode({
      service: Value,
      layer: Layer.effect(
        Value,
        Effect.map(LocationServiceMap.Service, () => Value.of({ value: "replacement" })),
      ),
    ...
    const layer = AppNodeBuilder.build(result, [[value, replacement]])
