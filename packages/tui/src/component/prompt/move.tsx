...
export function usePromptMove(input: { projectID: () => string | undefined; sessionID: () => string | undefined }) {
  ...
  async function create(context?: string) {
    ...
    try {
      const generated = await sdk.native.projectCopies.generateName({ projectID, context })
      const result = await sdk.native.projectCopies.create({
        projectID,
      ...
      // before moving on
      await sdk.native.location.get({ location: { directory } })
  ...
  async function moveExistingSession(sessionID: string, selection: MoveSessionSelection) {
    ...
    try {
      await sdk.native.controlPlane.moveSession({
        sessionID,
      ...
      await sdk.native.sessions
        .prompt({
          sessionID,
          prompt: { text: "", context: [{ text: moveReminderText(directory) }] },
          resume: false,
        })
        .catch(() => undefined)
      dialog.clear()
