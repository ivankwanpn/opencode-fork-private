...
import type { TuiFileDiff, TuiPlugin, TuiPluginApi, TuiRouteCurrent } from "@opencode-ai/plugin/tui"
import {
  TextAttributes,
  type BorderSides,
  type BoxRenderable,
...
const normalizeDiffs = (diffs: readonly TuiFileDiff[]): DiffFile[] =>
  diffs.flatMap((item) =>
    item.file
      ? [
          {
...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const [diff] = createResource(diffInput, async (input) => {
    if (input.mode === "last-turn") {
      ...
      if (!sessionID) return []
      const result = await props.api.client.sessions.diff({ sessionID, messageID: input.messageID })
      return normalizeDiffs(result)
    }
