...
describe("session.message-v2.latest", () => {
  ...
  test("uses message time when deterministic IDs do not sort lexically", () => {
    const userID = MessageID.make("msg_task_deterministic_input")
    const assistantID = MessageID.make("msg_00000000000000000000000000")
    const user: SessionV1.WithParts = {
      info: { ...userInfo(userID), time: { created: 1 } },
      parts: [
        {
          ...basePart(userID, "task"),
          type: "subtask",
          prompt: "inspect",
          description: "inspect",
          agent: "general",
    ...
    const assistant: SessionV1.WithParts = {
      info: { ...assistantInfo(assistantID, userID), time: { created: 2 }, finish: "stop" },
      parts: [],
    }

    const state = MessageV2.latest([user, assistant])
...
describe("session.message-v2.toLegacy", () => {
  test("projects canonical user, assistant, tool, and shell messages into the V1 response schema", () => {
    ...
    const messages: SessionMessage.Message[] = [
      ...
      SessionMessage.Assistant.make({
        ...
        content: [
          SessionMessage.AssistantText.make({ type: "text", id: "text", text: "done" }),
          SessionMessage.AssistantTool.make({
            type: "tool",
            id: "call",
            name: "read",
            time: { created: time, ran: time, completed: time },
            state: SessionMessage.ToolStateCompleted.make({
              status: "completed",
              input: { path: "file.ts" },
              content: [{ type: "text", text: "content" }],
              structured: { title: "Read file" },
            }),
          }),
        ],
      }),
      SessionMessage.Shell.make({
        ...
        type: "shell",
        userID: SessionMessage.ID.make("msg_requested_shell_user"),
        callID: "shell-call",
        command: "pwd",
        output: "/project",
        time: { created: time, completed: time },
    ...
    const projected = MessageV2.toLegacy(session, messages)
    expect(projected).toHaveLength(4)
    expect(projected[0]).toMatchObject({
      info: { id: "msg_user", role: "user", agent: "build", format: { type: "json_schema" } },
    ...
    expect(projected[1]).toMatchObject({
      ...
      parts: [
        { type: "text", text: "done" },
        { type: "tool", state: { status: "completed", title: "Read file", output: "content" } },
        { type: "step-finish", reason: "stop" },
      ],
    })
    expect(projected[3]).toMatchObject({
      info: { id: "msg_shell", role: "assistant", parentID: "msg_requested_shell_user" },
      parts: [{ type: "tool", callID: "shell-call", state: { output: "/project" } }],
    })
