...
const replyCompatible = (input: Parameters<Permission.Interface["reply"]>[0]) =>
  Effect.gen(function* () {
    const permission = yield* Permission.Service
    return yield* Permission.replyCompatible(permission, input)
  })

const canonicalPermission = (reply: PermissionV2.Interface["reply"]) =>
  PermissionV2.Service.of({
    ask: () => Effect.die("unused canonical permission ask"),
    assert: () => Effect.die("unused canonical permission assert"),
    reply,
...
const canonicalLocations = (service: PermissionV2.Interface) =>
  ({
    get: () => Layer.succeed(PermissionV2.Service, service),
  }) as never
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* replyCompatible({ requestID: PermissionV1.ID.make("per_test1"), reply: "once" })
...
it.instance(
  "reply compatibility falls back to the canonical registry",
  () =>
    Effect.gen(function* () {
      const replies: PermissionV2.ReplyInput[] = []
      const canonical = canonicalPermission((input) =>
        Effect.sync(() => {
          replies.push(input)
        }),
      )
      const provideCanonical = Effect.provideService(LocationServiceMap.Service, canonicalLocations(canonical))

      yield* replyCompatible({
        requestID: PermissionV1.ID.make("per_compat_once"),
        reply: "once",
      }).pipe(provideCanonical)
      yield* replyCompatible({
        requestID: PermissionV1.ID.make("per_compat_reject"),
        reply: "reject",
        message: "declined",
      }).pipe(provideCanonical)
      yield* replyCompatible({
        requestID: PermissionV1.ID.make("per_compat_always"),
        reply: "always",
      }).pipe(provideCanonical)

      expect(replies).toEqual([
        {
          requestID: PermissionV2.ID.create("per_compat_once"),
          reply: "once",
          message: undefined,
        },
        {
          requestID: PermissionV2.ID.create("per_compat_reject"),
          reply: "reject",
          message: "declined",
        },
        {
          requestID: PermissionV2.ID.create("per_compat_always"),
          reply: "always",
          message: undefined,
        },
      ])
...
it.instance(
  "reply compatibility preserves the legacy not found error",
  () =>
    Effect.gen(function* () {
      const requestID = PermissionV1.ID.make("per_compat_missing")
      const canonical = canonicalPermission((input) =>
        Effect.fail(
          new PermissionV2.NotFoundError({
            requestID: input.requestID,
          }),
        ),
      )
      const exit = yield* replyCompatible({ requestID, reply: "once" }).pipe(
        Effect.provideService(LocationServiceMap.Service, canonicalLocations(canonical)),
        Effect.exit,
      )
