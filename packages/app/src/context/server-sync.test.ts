...
describe("provider refresh", () => {
  ...
  test("refetches bootstrap before invalidating global and directory provider queries", async () => {
    ...
    expect(calls).toEqual(["bootstrap", "invalidate", "invalidate"])
    expect(filters[0]?.queryKey).toEqual([ServerScope.local, null, "providers"])
    expect(filters[1]?.predicate?.({ queryKey: [ServerScope.local, "/project", "providers"] })).toBe(true)
    expect(filters[1]?.predicate?.({ queryKey: [ServerScope.local, "/project", "sessions"] })).toBe(false)
    expect(filters[1]?.predicate?.({ queryKey: ["remote", "/project", "providers"] })).toBe(false)
  })

  test("waits for both provider invalidations", async () => {
    const pending: Array<() => void> = []
    let complete = false
    const refresh = refreshProviderQueries({
      bootstrap: { refetch: async () => undefined },
      queryClient: {
        invalidateQueries: () =>
