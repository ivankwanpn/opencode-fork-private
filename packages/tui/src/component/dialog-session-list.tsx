...
export function DialogSessionList() {
  ...
  const listSessions = async (input: { search?: string; filter: SessionListFilter }) => {
  ...
  const [browseResults, { refetch: refetchBrowse }] = createResource(
    () => sync.session.query(),
    (filter) => listSessions({ filter }).catch(() => undefined),
  )
  const [searchResults, { refetch }] = createResource(
    () => ({ query: search(), filter: sync.session.query() }),
    (input) => {
      if (!input.query) return undefined
      return listSessions({ search: input.query, filter: input.filter }).catch(() => undefined)
    },
  )
  ...
  function recover(session: NonNullable<ReturnType<typeof sessions>[number]>) {
    ...
    const warp = async (selection: WorkspaceSelection) => {
      const workspaceID = await (async () => {
        ...
        let result
        try {
          result = await sdk.native.workspaces.create({ type: selection.workspaceType, branch: null })
        } catch (err) {
          toast.show({
        ...
        if (!workspace) {
          toast.show({
            title: "Failed to create workspace",
            message: errorMessage("no response"),
            variant: "error",
          })
          return
        }
