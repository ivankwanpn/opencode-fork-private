  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        function fromPlugin(contribution: PluginToolCompat.Contribution): Tool.Def {
          // Discovery and schema normalization are shared with the canonical V2
          // adapter; this boundary keeps only the existing V1 execution behavior.
          const id = contribution.id
          const def = contribution.definition
          return {
            id,
            parameters: contribution.parameters,
            jsonSchema: contribution.jsonSchema,
        ...
        custom.push(...(yield* pluginTools.list()).map(fromPlugin))
    ...
    const describeCodeMode = Effect.fn("ToolRegistry.describeCodeMode")(function* (input: {
      ...
      return codeMode.describeCatalog(tools)
    })
