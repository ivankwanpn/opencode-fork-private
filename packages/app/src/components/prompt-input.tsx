...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const protocols = createMemo(() => props.controls.model.selection.protocol.list())
  const showProtocolControl = createMemo(() => protocols().length > 1)
                      ...
                      >
                        <Tooltip placement="top" gutter={4} value={language.t("provider.custom.protocol.label")}>
                          <Select
                            size="normal"
                            options={protocols()}
                            current={props.controls.model.selection.protocol.current()}
                            label={(value) =>
                            ...
                            onSelect={(value) => {
                              if (!value) return
                              props.controls.model.selection.protocol.set(value)
                              restoreFocus()
                            }}
                            class="max-w-[180px] text-text-base"
                            valueClass="truncate text-13-regular text-text-base"
                            triggerStyle={control()}
                            triggerProps={{ "data-action": "prompt-model-protocol" }}
                            variant="ghost"
                          />
                        </Tooltip>
                      </div>
