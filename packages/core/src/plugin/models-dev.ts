...
function applyModel(
  ...
  input: {
    ...
    readonly provider?: Pick<ModelsDev.Provider, "api" | "npm">
  } = {},
) {
  ...
  draft.variants = VariantPlugin.generate(
    draft.api.type === "native" && input.provider?.npm
      ? {
          ...draft,
          api: {
            id: draft.api.id,
            type: "aisdk",
            package: input.provider.npm,
        ...
        for (const item of Object.values(data)) {
          ...
          for (const model of Object.values(item.models)) {
            ...
            catalog.model.update(providerID, model.id, (draft) =>
              applyModel(draft, model, { cost: baseCost, provider: item }),
            )
