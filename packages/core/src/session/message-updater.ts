...
export function update(adapter: Adapter, event: SessionEvent.Event) {
  ...
  return Effect.gen(function* () {
    yield* SessionEvent.All.match(event, {
      ...
      "session.next.error": () => Effect.void,
      ...
      "session.next.input.terminalized": () => Effect.void,
      ...
      "session.next.shell.delta": () => Effect.void,
      ...
      "session.next.tool.success": (event) => {
        return updateOwnedAssistant(event.data.assistantMessageID, (draft) => {
          const match = latestTool(draft, event.data.callID)
          if (match && match.state.status === "running") {
            if (match.name === STRUCTURED_OUTPUT_TOOL_NAME) draft.structured = event.data.structured
            match.provider = {
      ...
      "session.next.provider.attempt.response.started": () => Effect.void,
      "session.next.provider.attempt.ended": () => Effect.void,
      "session.next.provider.recovery.decided": () => Effect.void,
