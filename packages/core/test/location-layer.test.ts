...
describe("LocationServiceMap", () => {
  ...
  it.live("invalidates every loaded location", () =>
    Effect.acquireRelease(
      ...
      Effect.flatMap(([firstDir, secondDir]) => {
        ...
        const project = Layer.succeed(
          ...
          Project.Service.of({
            ...
            resolve: (directory) =>
              Effect.sync(() => {
                boots.set(directory, (boots.get(directory) ?? 0) + 1)
                return { id: Project.ID.global, directory }
              }),
            commit: () => Effect.void,
          }),
        )
        const first = Location.Ref.make({ directory: AbsolutePath.make(firstDir.path) })
        const second = Location.Ref.make({ directory: AbsolutePath.make(secondDir.path) })

        return Effect.gen(function* () {
          const locations = yield* LocationServiceMap.Service
          const read = (ref: Location.Ref) => locations.contextEffect(ref)
      ...
      Effect.flatMap((dir) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            const explicitUndefined = Location.Ref.make({ directory, workspaceID: undefined })
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
                ...
                plugins: [
                  {
                    ...
                    options: { description: "Ready at location startup" },
                  },
                ],
              }),
            ),
          ...
          const configured = yield* AgentV2.Service.use((agents) =>
            agents.get(AgentV2.ID.make("effect-configured")),
          ).pipe(Effect.provide(LocationServiceMap.Service.get(location)))

          expect(configured).toMatchObject({
            description: "Ready at location startup",
            mode: "subagent",
          })
        }),
      ),
  ...
  it.live("resolves a migrated V1 provider immediately after location startup", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (dir) => Effect.promise(() => dir[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          const resolved = yield* SessionRunnerModel.Service.use((models) =>
            models.resolveWithInfo!(
              SessionV2.Info.make({
                id: SessionV2.ID.make("ses_migrated_v1_model"),
          ...
          expect(resolved.provider.id).toBe(ProviderV2.ID.make("test"))
        }),
      ),
    ),
  )
