...
const TOOL_FILE_MIMES = new Set<string>([...ProviderShared.IMAGE_MIMES, "image/jpg"])
const USER_FILE_MIMES = new Set<string>([
  "application/pdf",
  "text/csv",
  "application/msword",
...
const lowerToolResultContent = Effect.fn("BedrockConverse.lowerToolResultContent")(function* (
  items: ReadonlyArray<ToolContent>,
) {
...
const lowerToolResult = Effect.fn("BedrockConverse.lowerToolResult")(function* (part: ToolResultPart) {
  if (part.result.type === "text" || part.result.type === "error")
    return {
      tool: {
        toolResult: {
          toolUseId: part.id,
          content: [{ text: ProviderShared.toolResultText(part) }],
  ...
  if (part.result.type === "json")
    return {
      tool: {
        toolResult: {
          toolUseId: part.id,
          content: [{ json: part.result.value }],
          status: "success",
        },
      } satisfies BedrockToolResultBlock,
      user: [],
  ...
  const documents: Array<Schema.Schema.Type<typeof BedrockMedia.DocumentBlock>> = []
  for (const item of [...partition.user, ...partition.unsupported]) {
    const media = yield* BedrockMedia.lower({
      type: "media",
      mediaType: item.mime,
      data: item.uri,
    ...
    if (!("document" in media))
      return yield* ProviderShared.invalidRequest("Bedrock Converse tool fallback requires document media")
  ...
  return {
    tool: {
      toolResult: {
        toolUseId: part.id,
        content: yield* lowerToolResultContent(partition.tool),
        status: "success",
      },
    } satisfies BedrockToolResultBlock,
    user: documents,
...
const lowerMessages = Effect.fn("BedrockConverse.lowerMessages")(function* (
  ...
  for (const message of request.messages) {
    ...
    for (const part of message.content) {
      ...
      content.push(result.tool)
      documents.push(...result.user)
