...
const lowerMessages = Effect.fn("Gemini.lowerMessages")(function* (request: LLMRequest) {
  ...
  const fallback: Array<Schema.Schema.Type<typeof GeminiInlineDataPart>> = []
  const flushFallback = () => {
    if (fallback.length === 0) return false
    contents.push({ role: "user", parts: fallback.splice(0) })
    return true
  }

  for (const message of request.messages) {
    const fallbackFlushed = message.role === "tool" ? false : flushFallback()
    ...
    for (const part of message.content) {
      ...
      const partition = ProviderShared.partitionToolContent(
        ...
        (item) => MEDIA_MIMES.has(item.mime.toLowerCase()),
      )
      if (partition.unsupported.length > 0)
        return yield* ProviderShared.invalidRequest(
          `Gemini does not support media type ${partition.unsupported[0]!.mime}`,
        )
      ...
      for (const item of partition.user) {
        const media = yield* ProviderShared.validateToolFile("Gemini", item, MEDIA_MIMES)
        fallback.push({ inlineData: { mimeType: media.mime, data: media.base64 } })
      }
    }
    contents.push({ role: "user", parts })
  }
