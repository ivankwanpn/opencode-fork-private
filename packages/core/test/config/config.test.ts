...
describe("Config", () => {
  ...
  it.effect("preserves the v1 subagent depth limit", () =>
    Effect.sync(() => {
  ...
  it.live("loads inline V1 config at highest priority", () => {
    const contents = JSON.stringify({
      ...
      provider: {
        test: {
          ...
          options: { apiKey: "test-key", baseURL: "http://127.0.0.1:3000/v1" },
          models: {
            "test-model": {
              id: "test-model",
              name: "Test Model",
    ...
    return Effect.acquireUseRelease(
      Effect.sync(() => {
        ...
        process.env.OPENCODE_CONFIG_CONTENT = contents
        return previous
      }),
      () =>
        Effect.acquireRelease(
          ...
          Effect.flatMap((tmp) =>
            Effect.gen(function* () {
              yield* Effect.promise(() =>
                fs.writeFile(path.join(tmp.path, "opencode.json"), JSON.stringify({ model: "file/model" })),
              )
              const config = yield* Config.Service
              const entries = yield* config.entries()
              const inline = entries.findLast(
                (entry): entry is Config.Document => entry.type === "document" && entry.path === undefined,
              )

              expect(Config.latest(entries, "model")).toBe("test/test-model")
              expect(inline?.info.providers?.test?.api).toMatchObject({
                type: "aisdk",
