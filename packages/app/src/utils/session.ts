...
import { extractArray } from "./response-helpers"
...
export async function listAllSessions(api: Pick<SessionApi, "list">, input: Omit<SessionListInput, "cursor">) {
  const load = async (cursor?: string): Promise<Session[]> => {
    ...
    const sessions = extractArray(result).map(normalizeSessionInfo)
    if (extractArray(result).length === 0 || !result.cursor.next) return sessions
