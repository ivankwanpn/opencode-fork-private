...
import { PluginRuntime as PluginHooks } from "./plugin/runtime"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const add = Effect.fn("Plugin.add")(function* (id: ID, effect: PluginRuntime["effect"]) {
      ...
      const pluginOrder = order.get(id) ?? nextOrder++
      order.set(id, pluginOrder)
            ...
            State.batch(
              Effect.gen(function* () {
                ...
                yield* effect(host).pipe(
                  Effect.provideService(PluginHooks.CurrentOwner, { id, order: pluginOrder }),
                  Scope.provide(child),
...
).pipe(Layer.provideMerge(PluginHooks.locationLayer))
