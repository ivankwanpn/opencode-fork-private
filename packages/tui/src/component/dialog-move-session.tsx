import { isProjectCopyError } from "@opencode-ai/client"
...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const [loadedProject] = createResource(
    ...
    (projectID) =>
      sdk.native.projects
        ...
        .then((result) => result.data.findLast((item) => item.strategy === undefined)?.directory)
        .catch(() => undefined),
  )
  ...
  const [directories, { refetch }] = createResource(
    ...
    async (projectID, info): Promise<ProjectDirectory[] | undefined> => {
      try {
        await sdk.native.projectCopies.refresh({
          projectID,
          location: { directory: sdk.directory },
        })
        const directories = await sdk.native.projects.directories({ projectID })
        setLoadError(undefined)
        return directories.data.map((item) => ({ ...item }))
      } catch (error) {
        setLoadError(error)
  ...
  async function remove(option: DialogSelectOption<MoveSessionSelection | undefined>) {
    ...
    setWorking(true)
    const result = await sdk.native.projectCopies
      .remove({
    ...
    if (result.error) {
      ...
      setWorking(false)
      if (isProjectCopyError(result.error) && result.error.data.forceRequired) {
        ...
        const forced = await sdk.native.projectCopies
          .remove({
            projectID: props.projectID,
            location: { directory: sdk.directory },
            directory: selected.directory,
