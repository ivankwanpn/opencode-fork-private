  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(() => reject(new Error(`timed out waiting for ${type}; saw: ${seen.join(", ")}`)), 5_000)
      }),
    ])
  } finally {
...
describe("v2 location HttpApi", () => {
  ...
  test("streams native EventV2 payloads with location metadata", async () => {
    await using publisher = await tmpdir({ git: true })
    const response = await request("/api/event", publisher.path)
