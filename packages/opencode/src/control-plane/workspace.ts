...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const setStatus = Effect.fn("Workspace.setStatus")(function* (
      id: WorkspaceV2.ID,
      ...
      yield* events.publish(Event.Status, next, {
        location: { directory: AbsolutePath.make("global"), workspaceID: id },
      })
    })
    ...
    const syncWorkspaceLoop = Effect.fn("Workspace.syncWorkspaceLoop")(function* (space: Info) {
      ...
      while (true) {
        ...
        const stream = yield* connectSSE(target.url, target.headers).pipe(
          ...
          Effect.catch((err) =>
            Effect.gen(function* () {
              yield* setStatus(space.id, "error")
    ...
    const startSync = Effect.fn("Workspace.startSync")(function* (space: Info) {
      ...
      if (target.type === "local") {
        yield* setStatus(space.id, (yield* fs.existsSafe(target.directory)) ? "connected" : "error")
        return
      }
    ...
    const startWorkspaceSyncing = Effect.fn("Workspace.startWorkspaceSyncing")(function* (projectID: ProjectV2.ID) {
      ...
      for (const { workspace } of rows) {
        yield* startSync(fromRow(workspace)).pipe(
          Effect.catch(() => setStatus(workspace.id, "error")),
          Effect.forkDetach,
        )
      }
    })
