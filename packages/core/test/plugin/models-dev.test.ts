...
describe("ModelsDevPlugin", () => {
  it.effect("projects models.dev modes as separate models instead of variants", () =>
    Effect.gen(function* () {
      ...
      expect(base?.variants.map((variant) => ({ ...variant, id: String(variant.id) }))).toEqual([
        { id: "none", headers: {}, body: { reasoningEffort: "none" } },
      ...
      expect(base?.capabilities).toMatchObject({
        tools: true,
        temperature: true,
        reasoning: true,
        attachment: false,
      ...
      expect(fast?.variants.map((variant) => ({ ...variant, id: String(variant.id) }))).toEqual([
        { id: "none", headers: {}, body: { reasoningEffort: "none" } },
        { id: "low", headers: {}, body: { reasoningEffort: "low" } },
        { id: "medium", headers: {}, body: { reasoningEffort: "medium" } },
        { id: "high", headers: {}, body: { reasoningEffort: "high" } },
