...
type BootContext = Pick<
  RunInput,
  "sdk" | "native" | "directory" | "sessionID" | "sessionTitle" | "resume" | "agent" | "model" | "variant"
>
...
type RunLocalInput = {
  ...
  share: (sdk: RunInput["sdk"], native: RunInput["native"], sessionID: string) => Promise<void>
...
function createSessionResolver(fn?: CreateSession) {
  ...
  return async (ctx: BootContext, input: CreateSessionInput): Promise<ResolvedSession> => {
    const created = await fn(ctx.sdk, ctx.native, input)
    if (!created.id) {
...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  ...
  const shell = await (deps.createRuntimeLifecycle ?? createRuntimeLifecycle)({
    ...
    onPermissionReply: async (next) => {
      ...
      await ctx.native.permissions.reply({
        ...next,
        sessionID: state.sessionID,
      })
    },
    onQuestionReply: async (next) => {
      ...
      await ctx.native.questions.reply({
        ...next,
        sessionID: state.sessionID,
      })
    },
    ...
    onBackground: () => {
      ...
      void ctx.native.sessions.background({ sessionID: state.sessionID }).catch(() => {})
    },
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  ...
  const sdk = createNativeCompatClient({ native, directory: input.directory })
  ...
  return runInteractiveRuntime({
    ...
    resolveSession: () => {
      ...
      session = Promise.all([input.resolveAgent(), input.session(sdk, native)]).then(([agent, next]) => {
        if (!next?.id) {
          throw new Error("Session not found")
        }

        void input.share(sdk, native, next.id).catch(() => {})
        return {
          sessionID: next.id,
