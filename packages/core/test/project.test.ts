...
const it = testEffect(AppNodeBuilder.build(LayerNode.group([ProjectV2.node, Database.node, EventV2.node])))
...
describe("ProjectV2.update", () => {
  it.live("updates project metadata and publishes the updated project", () =>
    Effect.gen(function* () {
      const projectID = ProjectV2.ID.make("project-update")
      const worktree = AbsolutePath.make("D:\\project-update")
      ...
      yield* db
        .insert(ProjectTable)
        .values({ id: projectID, worktree, sandboxes: [], time_created: 1, time_updated: 1 })
        .run()
        .pipe(Effect.orDie)

      const result = yield* ProjectV2.update({
        projectID,
        name: "Renamed",
        icon: { color: "blue" },
      ...
      expect(published.find((event) => event.type === Project.Event.Updated.type)?.data).toEqual(result)
    }),
  )
})
