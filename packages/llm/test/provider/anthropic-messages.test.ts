...
describe("Anthropic Messages route", () => {
  ...
  it.effect("merges parallel tool results into the next Anthropic user message", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          model,
          messages: [
            Message.user("Check both."),
            Message.assistant([
              ToolCallPart.make({ id: "call_1", name: "lookup", input: { query: "first" } }),
              ToolCallPart.make({ id: "call_2", name: "lookup", input: { query: "second" } }),
            ]),
            Message.tool({ id: "call_1", name: "lookup", result: "First.", resultType: "text" }),
            Message.tool({ id: "call_2", name: "lookup", result: "Second.", resultType: "text" }),
          ],
          cache: "none",
        }),
      )
      ...
      expect(prepared.body.messages).toMatchObject([
        { role: "user", content: [{ type: "text", text: "Check both." }] },
        {
          role: "assistant",
          content: [
            { type: "tool_use", id: "call_1", name: "lookup", input: { query: "first" } },
            { type: "tool_use", id: "call_2", name: "lookup", input: { query: "second" } },
          ],
        },
        {
          role: "user",
  ...
  it.effect("merges consecutive tool results before ordered PDF documents", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          model,
          messages: [
            Message.assistant([
              ...
              ToolCallPart.make({ id: "call_2", name: "fetch", input: {} }),
            ]),
            Message.tool({
              id: "call_1",
              name: "read",
              metadata: { mcpMeta: { transient: true } },
              resultType: "content",
              result: [
                ...
                { type: "file", uri: "data:application/pdf;base64,UERGREFUQQ==", mime: "application/pdf" },
              ],
            }),
            Message.tool({
              id: "call_2",
              ...
              result: [
                ...
                { type: "file", uri: "data:application/pdf;base64,UERGREFUQg==", mime: "application/pdf" },
              ],
            }),
          ],
        }),
      ...
      expect(prepared.body.messages.slice(-2)).toMatchObject([
        {
          role: "user",
          content: [
            {
            ...
            { type: "tool_result", tool_use_id: "call_2", content: [{ type: "text", text: "Fetched." }] },
          ],
        },
        {
          role: "user",
          content: [
            ...
            { type: "document", source: { type: "base64", media_type: "application/pdf", data: "UERGREFUQg==" } },
          ],
        },
      ])
      expect(JSON.stringify(prepared.body)).not.toContain("provenance")
      expect(JSON.stringify(prepared.body)).not.toContain("mcpMeta")
      expect(JSON.stringify(prepared.body)).not.toContain("annotations")
      expect(JSON.stringify(prepared.body)).not.toContain('"meta"')
      expect(JSON.stringify(prepared.body)).not.toContain('"trace"')
    }),
  )
