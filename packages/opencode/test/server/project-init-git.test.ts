...
describe("project.initGit endpoint", () => {
  it.instance("initializes and disposes through the current lifecycle API", () =>
    Effect.gen(function* () {
      const tmp = yield* TestInstance
      const events = yield* collectGlobalEvents()
      const location = `location%5Bdirectory%5D=${encodeURIComponent(tmp.directory)}`

      const init = yield* request(tmp.directory, `/api/project/git/init?${location}`, {
        method: "POST",
      })
      ...
      const dispose = yield* request(tmp.directory, `/api/location?${location}`, {
        method: "DELETE",
      })
      expect(dispose.status).toBe(204)
      expect(disposedEvents(events.seen, tmp.directory)).toBe(2)
    }),
  )
