...
describe("plugin.loader.shared", () => {
  ...
  it.live("delegates V1 plugin identity and lifecycle ownership to the location V2 host", () =>
    withTmp(
      async (dir) => {
        ...
        const source = (name: string) =>
          [
            ...
            `    dispose: async () => Bun.write(${JSON.stringify(disposed)}, ${JSON.stringify(name)}),`,
            "  }),",
            "}",
            "",
          ].join("\n")
        await Bun.write(first, source("first"))
        await Bun.write(second, source("second"))
        await Bun.write(
          path.join(dir, "opencode.json"),
      ...
      (tmp) =>
        Effect.gen(function* () {
          const added: string[] = []
          const removed: string[] = []
          const hooks = yield* load(tmp.path, undefined, recordingLocationServiceMap(added, removed))

          expect(added).toEqual(["demo.shared", "demo.shared"])
          expect(hooks).toHaveLength(1)
          expect((hooks[0] as { name?: string }).name).toBe("second")

          yield* disposeAllInstancesEffect

          expect(removed).toEqual(["demo.shared", "demo.shared"])
          const legacyDisposeRan = yield* Effect.promise(() =>
            fs
              .readFile(tmp.extra.disposed, "utf8")
              .then(() => true)
              .catch(() => false),
          )
          expect(legacyDisposeRan).toBe(false)
        }),
    ),
  )
