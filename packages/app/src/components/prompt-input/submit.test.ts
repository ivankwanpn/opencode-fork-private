...
const sessionCreateInputs: Array<{
  ...
  model?: {
    ...
    providerID: string
    variant?: string
    protocol?: "openai-responses" | "openai-compatible" | "anthropic-messages"
  }
...
describe("prompt submit worktree selection", () => {
  test("reads the latest worktree accessor value per submit", async () => {
    ...
    expect(sessionCreateInputs).toEqual([
      {
        agent: "agent",
        model: { id: "model", providerID: "provider", variant: undefined, protocol: undefined },
    ...
    expect(sentCommands).toEqual([
      {
        ...
        agent: "agent",
        model: { id: "model", providerID: "provider", variant: "high", protocol: undefined },
        files: [],
      },
    ])
    expect(serverSessionSyncs).toBe(0)
  ...
  test("uses an injected model selection", async () => {
    ...
    const model = {
      ...
      protocol: { current: () => "anthropic-messages" },
