...
function normalizePath(input: string) {
  ...
  const resolved = path.win32.normalize(path.win32.resolve(input))
  try {
...
export async function create(input: {
  ...
  function waitForFreshPush(request: { path: string; version: number; after: number; timeout: number }) {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      unsubscribe = () => diagnosticListeners.delete(listener)
      schedule()
    })
  }
  ...
  const result = {
    ...
    notify: {
      async open(request: { path: string }) {
        ...
        const text = await Bun.file(request.path).text()
        const extension = path.extname(request.path)
        const languageId = LSPLanguage.LANGUAGE_EXTENSIONS[extension] ?? "plaintext"
    ...
    async shutdown() {
      connection.end()
      connection.dispose()
      await LSPProcess.stop(input.server.process)
    },
  }
