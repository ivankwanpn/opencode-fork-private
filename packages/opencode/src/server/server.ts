...
type WebHandler = ReturnType<typeof HttpApiApp.webHandler>["handler"]

const app = (handler: WebHandler) => {
  const result: ServerApp = {
    fetch: (request: Request) => handler(request, HttpApiApp.context),
    request(input, init) {
      return result.fetch(input instanceof Request ? input : new Request(new URL(input, "http://localhost"), init))
    },
  }
  return { app: result }
}
...
export const Native = lazy(() => {
  const handler = HttpApiApp.nativeWebHandler().handler
  return app(handler)
})
...
async function runListener(effect: Effect.Effect<EffectListener, unknown>): Promise<Listener> {
  const listener = await Effect.runPromise(effect)
  return {
