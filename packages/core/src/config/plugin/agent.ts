  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.agent.transform(
      Effect.fn(function* (draft) {
        ...
        for (const document of documents) {
          for (const [id, item] of Object.entries(document.info.agents ?? {})) {
            ...
            draft.update(agentID, (agent) => {
              ...
              if (item.protocol !== undefined && agent.model !== undefined) agent.model.protocol = item.protocol
        ...
        for (const current of draft.list()) {
          if (current.mode !== "subagent") continue
          draft.update(current.id, (agent) => {
            if (!agent.permissions.some((rule) => rule.action === "todowrite"))
              agent.permissions.push({ action: "todowrite", resource: "*", effect: "deny" })
            if (!agent.permissions.some((rule) => rule.action === "task"))
              agent.permissions.push({ action: "task", resource: "*", effect: "deny" })
          })
        }
      }),
    )
