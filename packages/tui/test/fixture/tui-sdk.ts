...
export function createFetch(override?: FetchHandler, events?: ReturnType<typeof createEventSource>) {
  ...
  const fetch = (async (input: RequestInfo | URL) => {
    ...
    if (url.pathname === "/api/project/proj_test/directory") return located([{ directory: worktree }])
    if (url.pathname === "/api/provider/catalog")
      return located({ providers: [], models: [], connected: [], default: {} })
    if (
    ...
    if (url.pathname === "/api/console") return located({ consoleManagedProviders: [], switchableOrgCount: 0 })
    ...
    if (["/api/workspace", "/api/workspace/status", "/api/workspace/adapter"].includes(url.pathname)) return located([])
    if (url.pathname === "/project/current") return json({ id: "proj_test" })
    if (url.pathname === "/api/reference") return located([])
