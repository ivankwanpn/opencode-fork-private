...
import { LLM, LLMError, Message, Model, ToolCallPart, ToolResultPart, Usage } from "../../src"
...
describe("OpenAI Responses route", () => {
    ...
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          ...
          toolDiscoveries: [
            {
              assistantMessageID: "assistant-search-1",
              callID: "search-1",
              query: "calendar",
              limit: 8,
  ...
  it.effect("keeps repeated provider-local search call ids scoped to their assistant messages", () =>
    Effect.gen(function* () {
      const deferredChatTool = {
        name: "chat_search",
        ...
        LLM.request({
          ...
          messages: [
            Message.make({
              ...
              role: "assistant",
              content: ToolCallPart.make({ id: "duplicate", name: "discover_tools", input: { query: "calendar" } }),
            }),
            Message.make({
              id: "assistant-calendar",
              role: "tool",
              content: ToolResultPart.make({
                id: "duplicate",
                name: "discover_tools",
                result: { matches: ["calendar_create"] },
              }),
            }),
            Message.make({
              id: "assistant-chat",
              role: "assistant",
              content: ToolCallPart.make({ id: "duplicate", name: "discover_tools", input: { query: "chat" } }),
            }),
            Message.make({
              id: "assistant-chat",
              role: "tool",
              content: ToolResultPart.make({
                id: "duplicate",
                name: "discover_tools",
                result: { matches: ["chat_search"] },
              }),
            }),
          ],
          tools: [discoveryTool, deferredCalendarTool, deferredChatTool],
          toolDiscoveries: [
            {
              assistantMessageID: "assistant-calendar",
              callID: "duplicate",
              query: "calendar",
              limit: 8,
              catalogRevision: "catalog-1",
      ...
      const outputs = prepared.body.input?.filter(
        (item): item is Extract<(typeof prepared.body.input)[number], { type: "tool_search_output" }> =>
          "type" in item && item.type === "tool_search_output",
      )
      expect(outputs).toHaveLength(2)
      expect(outputs?.[0]?.tools).toEqual([
        expect.objectContaining({
          type: "namespace",
          name: "calendar",
          tools: [expect.objectContaining({ name: "calendar_create" })],
        }),
      ])
      expect(outputs?.[1]?.tools).toEqual([
        expect.objectContaining({
          type: "namespace",
          name: "chat",
          tools: [expect.objectContaining({ name: "chat_search" })],
        }),
      ])
    }),
  )

  it.effect("does not classify an ordinary tool result by a search call id from another message", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model: nativeToolSearchModel,
          messages: [
            ...
            Message.make({
              id: "assistant-search",
              role: "tool",
              content: ToolResultPart.make({ id: "duplicate", name: "discover_tools", result: { matches: [] } }),
            }),
            Message.make({
              id: "assistant-function",
              role: "assistant",
              content: ToolCallPart.make({ id: "duplicate", name: "direct_lookup", input: { query: "today" } }),
            }),
            Message.make({
              id: "assistant-function",
              role: "tool",
              content: ToolResultPart.make({ id: "duplicate", name: "direct_lookup", result: { value: "done" } }),
            }),
          ],
          tools: [
            discoveryTool,
    ...
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          ...
          toolDiscoveries: [
            {
              assistantMessageID: "assistant-compacted",
  ...
  it.effect("repairs a durable native search whose chronological output is missing", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model: nativeToolSearchModel,
          messages: [
            Message.user("Find calendar tools."),
            Message.assistant([
              ToolCallPart.make({ id: "search-call-only", name: "discover_tools", input: { query: "calendar" } }),
            ]),
          ],
          tools: [discoveryTool, deferredCalendarTool],
          toolDiscoveries: [
            {
              assistantMessageID: "assistant-call-only",
              callID: "search-call-only",
              query: "calendar",
              limit: 8,
              catalogRevision: "catalog-1",
              tools: [deferredCalendarTool],
      ...
      expect(prepared.body.input).toEqual([
        ...
        expect.objectContaining({
          ...
          status: "completed",
          execution: "client",
          tools: [expect.objectContaining({ type: "namespace", name: "calendar" })],
        }),
      ])
    }),
  )
  ...
  it.effect("lowers image tool-result content as structured input_image items", () =>
    Effect.gen(function* () {
      ...
      expect(JSON.stringify(prepared.body)).not.toContain("provenance")
    }),
  )

  it.effect("lowers PDF tool files into the following user input item", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model,
          messages: [
            ...
            Message.tool({
              ...
              resultType: "content",
              result: [
                { type: "text", text: "Report read successfully" },
                {
                  type: "file",
      ...
      expect(prepared.body.input).toEqual([
        { type: "function_call", call_id: "call_pdf", name: "read", arguments: "{}" },
        {
          type: "function_call_output",
          call_id: "call_pdf",
          output: [{ type: "input_text", text: "Report read successfully" }],
        },
        {
          role: "user",
          content: [
        ...
        { role: "user", content: [{ type: "input_text", text: "Summarize the report." }] },
      ])
    }),
  )

  it.effect("preserves order for multiple PDF tool files in one user fallback", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model,
          messages: [
            Message.tool({
              ...
              resultType: "content",
              result: [
                { type: "file", uri: "data:application/pdf;base64,JVBERi0=", mime: "application/pdf", name: "one.pdf" },
                { type: "file", uri: "data:application/pdf;base64,JVBERi0x", mime: "application/pdf", name: "two.pdf" },
              ],
            }),
          ],
        }),
      ...
      expect(prepared.body.input).toEqual([
        { type: "function_call_output", call_id: "call_pdfs", output: [] },
        {
          role: "user",
          content: [
            { type: "input_file", filename: "one.pdf", file_data: "data:application/pdf;base64,JVBERi0=" },
            { type: "input_file", filename: "two.pdf", file_data: "data:application/pdf;base64,JVBERi0x" },
          ],
        },
      ])
    }),
  ...
  it.effect("keys native tool-search streams by call_id when the item id arrives late", () =>
    Effect.gen(function* () {
      const added = {
        type: "tool_search_call",
        call_id: "search-stable",
        execution: "client",
        arguments: { query: "calendar" },
      }
      const response = yield* LLMClient.generate(
        ...
        Effect.provide(
          fixedResponse(
            sseEvents(
              ...
              { type: "response.output_item.done", item: { ...added, id: "tsc-late", status: "completed" } },
      ...
      expect(response.events.filter((event) => event.type.startsWith("tool-"))).toEqual([
        {
          ...
          id: "search-stable",
          name: "discover_tools",
          providerMetadata: { openai: { execution: "client", toolSearch: true } },
        },
        {
          type: "tool-input-end",
          id: "search-stable",
  ...
  it.effect("ignores native tool-search items without a provider call_id", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(
        LLM.request({ model: nativeToolSearchModel, tools: [discoveryTool] }),
      ).pipe(
  ...
  it.effect("ignores provider-executed and null-id native search items", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(
        LLM.request({ model: nativeToolSearchModel, tools: [discoveryTool] }),
      ).pipe(
  ...
  it.effect("surfaces response.failed details from response.error", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toEqual([
        {
          type: "provider-error",
          message: "server_error: Upstream model unavailable (response_id=resp_failed_1)",
        },
      ])
    }),
  )
  ...
  it.effect("surfaces response.failed code when no nested message is present", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toEqual([
        { type: "provider-error", message: "invalid_prompt (response_id=resp_failed_2)" },
      ])
    }),
  )
  ...
  it.effect("prefers top-level nested error details when both payloads are present", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents({
              type: "error",
              error: { code: "top_level_error", message: "Authoritative failure" },
              response: { error: { code: "response_error", message: "Stale failure" } },
            }),
          ),
        ),
      )
      ...
      expect(response.events).toEqual([
        {
          type: "provider-error",
          message: "top_level_error: Authoritative failure",
        },
      ])
    }),
  )
  ...
  it.effect("falls back to a stable default when response.failed has no error payload", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toEqual([
        {
          type: "provider-error",
          message: "OpenAI Responses response failed (response_id=resp_failed_3)",
        },
      ])
    }),
  )

  it.effect("surfaces safe response.failed diagnostics", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents({
              ...
              response: {
                ...
                error: { code: "upstream_error", message: "Upstream request failed", param: "tools" },
              },
            }),
          ),
        ),
      ...
      expect(response.events).toEqual([
        {
          type: "provider-error",
          message:
            "upstream_error: Upstream request failed (status=failed, response_id=resp_failed_4, param=tools, reason=max_output_tokens)",
        },
      ])
    }),
  )
