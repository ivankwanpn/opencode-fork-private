...
function withClientNames(mcpTools: Record<string, MCP.McpTool>, servers: readonly string[]) {
  const longest = [...servers].sort((left, right) => right.length - left.length)
  return Object.fromEntries(
    Object.entries(mcpTools).map(([key, entry]) => {
      const clientName =
        entry.clientName ||
        longest.find((name) => key.startsWith(name + "_")) ||
        (key.includes("_") ? key.slice(0, key.indexOf("_")) : key)
      return [key, { ...entry, clientName }]
    }),
  )
}
...
function describeFor(mcpTools: Record<string, MCP.McpTool>, servers?: string[], permission: PermissionV1.Rule[] = []) {
  const normalized = withClientNames(mcpTools, serverNames(mcpTools, servers))
  return describeCatalog(Permission.visibleTools(normalized, permission))
}
...
describe("code mode execute", () => {
  ...
  test("asks permission before each child tool call", async () => {
    ...
    await Effect.runPromise(
      tool.execute({ code: "await tools.a.a({}); await tools.b.b({}); return 'done'" }, permissionCtx),
    )
  ...
  test("a denied permission fails the child call with a catchable message, not the whole execute", async () => {
    ...
    const output = await Effect.runPromise(
      tool.execute({ code: "try { await tools.a.a({}) } catch (e) { return 'denied: ' + e.message }" }, denyCtx),
    )

    expect(output.output).toBe("denied: permission denied by user")
    expect(output.metadata.error).toBeUndefined()
    expect(called).toEqual([])
    expect(output.metadata.toolCalls).toEqual([{ tool: "a.a", status: "error" }])
  })
  ...
  test("child calls fire plugin tool.execute hooks with the MCP key and synthetic parent/N call ids", async () => {
    ...
    const out = await Effect.runPromise(
      tool.execute({ code: "await tools.a.a({ x: 1 }); await tools.b.b({}); return 'done'" }, ctx),
    )
      ...
      tool.execute(
        {
          code: `
            let caught
            try { await tools.a.a({}) } catch (e) { caught = e.message }
            const r = await tools.b.b({})
            return caught + " / " + r
          `,
        },
        ctx,
