...
import type { Model as LegacyModel } from "@opencode-ai/sdk/v2/types"
...
import { PluginV1Projection } from "./plugin/v1-projection"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const available = (provider: ProviderV2.Info, integration: Integration.Info | undefined) => {
      ...
      if (ProviderV2.hasConfiguredCredentials(provider)) return true
    ...
    const result: Interface = {
      ...
      model: {
        ...
        small: Effect.fn("CatalogV2.model.small")(function* (providerID) {
          ...
          const selected = PluginRuntime.mutable<LegacyModel | undefined>(undefined)
          yield* plugins.run(PluginRuntime.HookName.providerSmallModel, {
            provider: PluginV1Projection.provider(
              provider,
              Array.fromIterable(record.models.values()).map((model) => projectModel(model, provider)),
              {
                source: PluginV1Projection.source(provider, connection),
                env:
                  integration?.methods.flatMap((method) =>
                    method.type === "env" ? method.names : [],
                  ) ?? [],
              },
            ),
            model: selected.value,
          })
          const experimental = selected.get()
          if (experimental) {
            const targetProvider = state.get().providers.get(ProviderV2.ID.make(experimental.providerID))
            const targetModel = targetProvider?.models.get(ModelV2.ID.make(experimental.id))
            if (targetProvider && targetModel) return projectModel(targetModel, targetProvider.provider)
          }
...
export const node = makeLocationNode({
  service: Service,
  layer,
  deps: [EventV2.node, Policy.node, Integration.node, PluginRuntime.node],
})
