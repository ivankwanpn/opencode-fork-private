...
import { makeLocationServiceMapReplacement } from "../lib/location-service-map"
...
const agentLocationServiceMapReplacement = makeLocationServiceMapReplacement((ref) => {
  const directory = AbsolutePath.make(path.resolve(ref.directory, "../docs"))
  return [
    new Reference.Info({
      name: "docs",
      path: directory,
...
const agentLayer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  LayerNode.compile(
    ...
    [[RuntimeFlags.node, RuntimeFlags.layer(flags)], agentLocationServiceMapReplacement],
  )
