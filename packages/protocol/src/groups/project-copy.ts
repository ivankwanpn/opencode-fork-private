...
export const ProjectCopyGroup = HttpApiGroup.make("server.projectCopy")
  .add(
    HttpApiEndpoint.post("projectCopy.generateName", `${root}/name`, {
      params: { projectID: Project.ID },
      query: LocationQuery,
      payload: Schema.Struct({ context: Schema.optional(Schema.String) }),
      success: Schema.Struct({ name: Schema.String }),
    })
      .annotateMerge(locationQueryOpenApi)
      .annotateMerge(OpenApi.annotations({ identifier: "v2.projectCopy.generateName" })),
  )
  .add(
