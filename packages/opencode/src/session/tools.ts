      ...
      execute: (args, opts) =>
        run.promise(
          Effect.gen(function* () {
            ...
            const raw = yield* interruptOnAbort(
              Effect.gen(function* () {
                yield* ctx.ask({ permission: key, metadata: {}, patterns: ["*"], always: ["*"] })
                return yield* McpCatalog.invoke(entry, record)
              }),
              opts.abortSignal,
            ).pipe(
            ...
            const result: CallToolResult & { readonly metadata?: Record<string, unknown> } =
              raw.content.length > 0 || raw.structuredContent === undefined || raw.structuredContent === null
                ? raw
                : {
                    ...raw,
                    content: [{ type: "text", text: JSON.stringify(raw.structuredContent) }],
                  }
            yield* plugin.trigger(
              "tool.execute.after",
              { tool: key, sessionID: ctx.sessionID, callID: opts.toolCallId, args: record },
              result,
            )
          ...
          for (const contentItem of result.content) {
            if (contentItem.type === "text") textParts.push(contentItem.text)
              ...
              if ("text" in resource) textParts.push(resource.text)
              if ("blob" in resource) {
...
function interruptOnAbort<A, E, R>(effect: Effect.Effect<A, E, R>, signal?: AbortSignal) {
