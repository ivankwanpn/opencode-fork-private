...
describe("plugin.codex", () => {
  ...
  test("applies Codex request parameter and header hooks", async () => {
    ...
    const params = {
      ...
      topP: 1,
      topK: 0,
      maxOutputTokens: 16_384 as number | undefined,
      options: {},
    }
    ...
    expect(headers.headers).toMatchObject({
      originator: "opencode",
      "session-id": "ses_codex",
    })
    expect(headers.headers["User-Agent"]).toContain("opencode/")
  })

  test("uses the live Codex model list for OAuth models", async () => {
    ...
    const provider = {
      models: {
        ...Object.fromEntries(
          ["gpt-live", "gpt-stale"].map((id) => [id, { id, api: { id }, limit, cost: {}, options: {} }]),
        ),
      },
    }
    ...
    const models = await hooks.provider!.models!(
      provider as never,
      {
        auth: { type: "oauth", access: "access-token", accountId: "account-123" },
      } as never,
    )

    expect(Object.keys(models)).toEqual(["gpt-live"])
    expect(models["gpt-live"]?.limit).toEqual({ context: 321_000, input: 922_000, output: 128_000 })
  ...
  test("deduplicates concurrent Codex token refreshes", async () => {
    ...
    expect(loaded.baseURL).toBe(new URL("/backend-api/codex", server.url).toString())
