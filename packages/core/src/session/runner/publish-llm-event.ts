...
export const createLLMEventPublisher = (events: EventV2.Interface, input: Input) => {
  ...
  const assistantMessageID = input.assistantMessageID ?? SessionMessage.ID.create()
  ...
  const currentAssistantMessageID = () =>
    assistantStarted ? Effect.succeed(assistantMessageID) : Effect.die("Tool event before assistant step start")
  ...
  const text = fragments("text", (textID, value) =>
    Effect.gen(function* () {
      ...
      const completed = input.textComplete
        ? yield* input.textComplete({ messageID: assistantMessageID, partID: textID, text: value })
        : value
  ...
  const failAssistant = Effect.fnUntraced(function* (
    message: string,
    kind: AssistantErrorCodec.Kind = "unknown",
  ) {
