...
export function Prompt(props: PromptProps) {
      ...
      },
      {
        ...
        run: () => {
          ...
          if (store.interrupt >= 2) {
            void sdk.native.sessions.interrupt({
  ...
  async function submitInner() {
    ...
    if (sessionID == null) {
      ...
      try {
        const created = await sdk.native.sessions.create({
          location: {
    ...
    if (store.mode === "shell") {
      ...
      const files = nonTextParts.filter((part) => part.type === "file")
      void sdk.native.sessions.command({
        sessionID,
      ...
      const send = async () => {
        ...
        if (
          ...
          (current.model.variant ?? "default") !== (variant ?? "default")
        )
          await sdk.native.sessions.switchModel({
            sessionID,
            model: { providerID: selectedModel.providerID, id: selectedModel.modelID, variant },
          })
        return sdk.native.sessions.prompt({
          sessionID,
          prompt: {
            text: inputText,
            context: editorParts.map((part) => ({ text: part.text, metadata: part.metadata })),
            files: nonTextParts
