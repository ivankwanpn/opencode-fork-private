...
const lowerImage = Effect.fn("AnthropicMessages.lowerImage")(function* (part: MediaPart) {
  const media = yield* ProviderShared.validateMedia("Anthropic Messages", part, TOOL_FILE_MIMES)
  return {
    type: "image" as const,
    source: {
...
const lowerToolResultContentItem = Effect.fn("AnthropicMessages.lowerToolResultContentItem")(function* (
  ...
  const media = yield* ProviderShared.validateToolFile("Anthropic Messages", item, TOOL_FILE_MIMES)
  return {
    type: "image" as const,
    source: {
      type: "base64" as const,
...
const lowerToolResultUserFile = Effect.fn("AnthropicMessages.lowerToolResultUserFile")(function* (
  item: ToolFileContent,
) {
  const media = yield* ProviderShared.validateToolFile("Anthropic Messages", item, USER_FILE_MIMES)
  return {
...
const lowerMessages = Effect.fn("AnthropicMessages.lowerMessages")(function* (
  ...
  const flushDocuments = () => {
    ...
    messages.push({ role: "user", content: documents.splice(0) })
    return true
  }

  for (const [index, message] of request.messages.entries()) {
    const documentsFlushed = message.role === "tool" ? false : flushDocuments()
    ...
    for (const part of message.content) {
      ...
      if (part.result.type === "content") {
        const result: ReadonlyArray<ToolContent> = part.result.value
        ...
        if (partition.unsupported.length > 0)
          return yield* invalid(`Anthropic Messages does not support media type ${partition.unsupported[0]!.mime}`)
        content.push({
          type: "tool_result",
          tool_use_id: part.id,
          content: yield* Effect.forEach(partition.tool, lowerToolResultContentItem),
          cache_control: cacheControl(breakpoints, part.cache),
        })
        documents.push(...(yield* Effect.forEach(partition.user, lowerToolResultUserFile)))
        continue
      }
      content.push({
        type: "tool_result",
    ...
    const previous = messages.at(-1)
    if (request.messages[index - 1]?.role === "tool" && previous?.role === "user") {
