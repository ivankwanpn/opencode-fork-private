...
import { isInvalidRequestError, type WorkspacesListAdaptersOutput } from "@opencode-ai/client"
...
type Adapter = WorkspacesListAdaptersOutput["data"][number]
...
async function loadWorkspaceAdapters(input: {
  ...
  try {
    const response = await input.sdk.native.workspaces.listAdapters({ location: { directory: dir } })
...
export async function openWorkspaceSelect(input: {
  ...
  input.dialog.clear()
  await input.sdk.native.workspaces.syncList().catch(() => undefined)
...
export async function warpWorkspaceSession(input: {
  ...
  try {
    await input.sdk.native.workspaces.warp({
      workspaceID: input.workspaceID,
      sessionID: input.sessionID,
      copyChanges: input.copyChanges,
    })
  } catch (err) {
    if (isInvalidRequestError(err) && err.kind === "workspace_vcs_apply") {
  ...
  if (dir) {
    await input.sdk.native.sessions
      .prompt({
        sessionID: input.sessionID,
        prompt: { text: "", context: [{ text: warpReminderText(dir) }] },
        resume: false,
      })
      .catch(() => undefined)
  }
...
export async function confirmWorkspaceFileChanges(input: {
  ...
  const status = await input.sdk.native.vcs
