...
describe("SessionRunCoordinator", () => {
  ...
  it.effect("runs exclusive work only while idle", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const started = yield* Deferred.make<void>()
        const gate = yield* Deferred.make<void>()
        ...
        const work = Deferred.succeed(started, undefined).pipe(Effect.andThen(Deferred.await(gate)))

        const running = yield* coordinator.exclusive("session", work).pipe(Effect.forkChild)
        yield* Deferred.await(started)
        expect(Array.from(yield* coordinator.active)).toEqual(["session"])
        expect((yield* coordinator.exclusive("session", Effect.void).pipe(Effect.flip))._tag).toBe(
          "SessionRunCoordinator.Busy",
        )
  ...
  it.effect("drains a wake registered during exclusive work", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const started = yield* Deferred.make<void>()
        const gate = yield* Deferred.make<void>()
  ...
  it.effect("returns immediately while idle without starting a drain", () =>
    Effect.scoped(
      Effect.gen(function* () {
        let runs = 0
        const coordinator = yield* SessionRunCoordinator.make({
  ...
  it.effect("waits across a coalesced successor without forcing a drain", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const firstStarted = yield* Deferred.make<void>()
        const firstGate = yield* Deferred.make<void>()
        ...
        yield* Deferred.await(firstStarted)
        yield* coordinator.wake("session")
        const waiting = yield* coordinator.wait("session").pipe(
  ...
  it.effect("waits across a replacement successor after execution failure", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const firstStarted = yield* Deferred.make<void>()
        const firstGate = yield* Deferred.make<void>()
