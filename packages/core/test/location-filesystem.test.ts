...
describe("FileSystem", () => {
  ...
  it.live("boots for a missing location and rejects filesystem access", () =>
    withTmp((directory) => {
      const missing = path.join(directory, "missing")
      return Effect.gen(function* () {
        const result = yield* (yield* FileSystem.Service).list().pipe(Effect.exit)
        expect(Exit.isFailure(result)).toBe(true)
      }).pipe(provide(missing))
    }),
  )
