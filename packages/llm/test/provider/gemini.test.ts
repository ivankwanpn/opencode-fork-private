...
describe("Gemini route", () => {
  ...
  it.effect("continues image tool results as inline vision input without base64 text", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.body.contents).toEqual([
        ...
        { role: "user", parts: [{ inlineData: { mimeType: "image/png", data: "AAECAw==" } }] },
      ])
      expect(JSON.stringify(prepared.body.contents)).not.toContain('"content":"AAECAw=="')
    }),
  )

  it.effect("keeps separate consecutive function responses contiguous before ordered audio and video", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<Gemini.GeminiBody>(
        LLM.request({
          model,
          messages: [
            Message.assistant([
              ToolCallPart.make({ id: "call_1", name: "record", input: {} }),
              ToolCallPart.make({ id: "call_2", name: "capture", input: {} }),
            ]),
            Message.tool({
              id: "call_1",
              name: "record",
              ...
              result: [
                ...
                {
                  type: "file",
                  uri: "data:audio/mp3;base64,AAECAw==",
                  mime: "audio/mp3",
                  provenance: {
                    type: "mcp",
            ...
            Message.tool({
              ...
              result: [
                ...
                { type: "file", uri: "data:video/mp4;base64,BAUGBw==", mime: "video/mp4" },
              ],
            }),
          ],
        }),
      ...
      expect(prepared.body.contents.slice(-3)).toEqual([
        {
          role: "user",
          parts: [{ functionResponse: { name: "record", response: { name: "record", content: "Audio ready." } } }],
        },
        {
          role: "user",
          parts: [{ functionResponse: { name: "capture", response: { name: "capture", content: "Video ready." } } }],
        },
        {
          role: "user",
          parts: [
            { inlineData: { mimeType: "audio/mp3", data: "AAECAw==" } },
            { inlineData: { mimeType: "video/mp4", data: "BAUGBw==" } },
          ],
        },
      ])
  ...
  it.effect("rejects PDF tool-result fallback media", () =>
    Effect.gen(function* () {
        ...
        LLM.request({
          ...
          messages: [
            Message.tool({
              ...
              name: "read",
              resultType: "content",
              result: [{ type: "file", uri: "data:application/pdf;base64,UERGREFUQQ==", mime: "application/pdf" }],
            }),
          ],
        }),
      ).pipe(Effect.flip)
  ...
  it.effect("strips matching data URLs to raw base64 inlineData", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.body.contents).toEqual([
        ...
        {
          role: "user",
          parts: [{ functionResponse: { name: "read", response: { name: "read", content: "" } } }],
        },
        { role: "user", parts: [{ inlineData: { mimeType: "image/jpeg", data: "/9j/" } }] },
      ])
    }),
  )
