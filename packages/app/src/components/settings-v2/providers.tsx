...
export const SettingsProvidersV2: Component<{ onBack?: () => void }> = (props) => {
  ...
  const isConfigCustom = (providerID: string) => {
    ...
    if (!provider.npm || !CUSTOM_PROVIDER_PACKAGES.has(provider.npm)) return false
                ...
                {(item) => (
                  <div class="settings-v2-provider-row group">
                    ...
                    <div class="flex items-center gap-1">
                      <Show when={canEditConnectedProvider(item)}>
                        <ButtonV2 size="normal" variant="ghost-muted" onClick={() => edit(item)}>
